...
/*!
   ...
   This is similar to \a strncpy, with two important differences:
    ...
    - the destination buffer is not filled with zeros past the copied string
       length
   These differences make it slightly more efficient, and safer to use since it
      will
   not leave the destination buffer unterminated. There is no need to pass an
      artificially
   reduced buffer size to this function (unlike \a strncpy), and the buffer does
      not need
...
/*!
   ...
   \note This method is not recommended. New code should use ast_str_*()
      instead.
 ...
 * This function checks to see whether a string passed to it is an indication of
 *an "true" value.
...
/*
   ...
   \param consumed The number of characters 'consumed' in the string by the
...
/*!
 * \brief Given a string regex_string in the form of "/regex/", convert it into
 *the form of "regex"
 *
 * This function will trim one leading / and one trailing / from a given input
 *string
 ...
 *      (&my_buf).
 * \param init_len This is the initial length of the thread's dynamic string.
 *The
 ...
 * \note Care should be taken when using this function. The function can
 * result in reallocating the ast_str. If a pointer to the ast_str is passed
 * by value to a function that calls ast_str_append_va(), then the original
 *ast_str
 * pointer may be invalidated due to a reallocation.
 *
 ...
 *         compared as such. Otherwise the result will be derived from
 *strcmp(left, right).
 * "regex":
 *     The right value will be compiled as a regular expression and matched
 *against the left
 *     value.
 * "like":
 *     Any '%' character in the right value will be converted to '.*' and the
 *resulting
 *     string will be handled as a regex.
 * NULL , "":
 *     If the right value starts and ends with a '/' then it will be processed
 *as a regex.
 *     Otherwise, same as "=".
 */
