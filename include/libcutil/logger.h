...
enum ast_logger_results {
  AST_LOGGER_SUCCESS     = 0,  /*!< Log channel was created or deleted
                                  successfully*/
  AST_LOGGER_FAILURE     = 1,  /*!< Log channel already exists for create or
                                  doesn't exist for deletion of log channel */
...
/*! \brief Used for sending a log message
        This is the standard logger function.  Probably the only way you will
           invoke it would be something like this:
        ast_log(AST_LOG_WHATEVER, "Problem with the %s Captain.  We should get
           some more.  Will %d be enough?\n", "flux capacitor", 10);
        where WHATEVER is one of ERROR, DEBUG, EVENT, NOTICE, or WARNING
           depending
        on which log you wish to output to. These are implemented as macros,
           that
 ...
 *
 * \note This function should be used by all error messages that might be
 * directly
...
/*! \brief Used for sending a log message with a known call_id
        This is a modified logger function which is functionally identical to
           the above logger function,
        it just include a call_id argument as well. If NULL is specified here,
        ...
        \param line	Will be provided by the AST_LOG_* macro
        \param function	Will be provided by the AST_LOG_* macro
        \param callid	This is the ast_callid that is associated with the log
           message. May be NULL.
        \param fmt	This is what is important.  The format is the same as
           your favorite breed of printf.  You know how that works, right? :-)
 */
 ...
 */
int ast_logger_get_channels(int (        *logentry)(
                              const char *channel,
                              const char *type,
                              const char *status,
 ...
 */
int ast_logger_remove_channel(
  const char *log_channel);

/*!
 * \brief Log a backtrace of the current thread's execution stack to the
 * Asterisk log
 */
 ...
 */
int                                        ast_logger_rotate_channel(
  const char *log_channel);

void __attribute__((format(printf, 5, 6))) ast_queue_log(const char *queuename,
                                                         const char *callid,
                                                         const char *agent,
                                                         const char *event,
                                                         const char *fmt,
 ...
 *
 * \details This works like ast_log, but prints verbose messages to the console
 * depending on verbosity level set.
 *
 * ast_verbose(VERBOSE_PREFIX_3 "Whatever %s is happening\n", "nothing");
 *
 * This will print the message to the console if the verbose level is set to a
 * level >= 3
 *
 ...
 */
void __attribute__((format(printf, 5, 6))) __ast_verbose(const char *file,
                                                         int         line,
                                                         const char *func,
                                                         int         level,
                                                         const char *fmt,
...
/*!
 * \brief Send a verbose message (based on verbose level) with deliberately
 * specified callid
 *
 * \details just like __ast_verbose, only __ast_verbose_callid allows you to
 * specify which callid is being used
 * for the log without needing to bind it to a thread. NULL is a valid argument
 * for this function and will
 * allow you to specify that a log will never display a call id even when there
 * is a call id bound to the
 * thread.
 */
void __attribute__((format(printf, 6, 7))) __ast_verbose_callid(const char *file,
                                                                int         line,
                                                                const char *func,
                                                                int         level,
                                                                ast_callid  callid,
...
void __attribute__((format(printf, 6, 0))) __ast_verbose_ap(const char *file,
                                                            int         line,
                                                            const char *func,
                                                            int         level,
                                                            ast_callid  callid,
...
void __attribute__((format(printf, 2, 3))) ast_child_verbose(int         level,
                                                             const char *fmt,
                                                             ...);
...
#endif // ifdef AST_LOG_VERBOSE
#define AST_LOG_VERBOSE    __LOG_VERBOSE, _A_

#ifdef LOG_DTMF
# undef LOG_DTMF
#endif // ifdef LOG_DTMF
#define __LOG_DTMF  6
#define LOG_DTMF    __LOG_DTMF, _A_

#ifdef AST_LOG_DTMF
# undef AST_LOG_DTMF
#endif // ifdef AST_LOG_DTMF
 ...
 */
unsigned int ast_verbose_get_by_module(const char *module) __attribute__((
                                                                           deprecated));
 ...
 * \retval non-zero level to be used with ast_log for sending messages to this
 * level
 * \since 1.8
 */
...
/*!
 * \brief Checks thread storage for a callid and stores a reference if it
 * exists.
 *        If not, then a new one will be created, bound to the thread, and a
 * reference
 ...
 *        references and if the callid was created by threadstorage_auto,
 * unbinds
 *        the callid from the threadstorage
 * \param callid The callid set by ast_callid_threadstorage_auto
 * \param callid_created The integer returned through
 ...
 *
 * \param buffer destination of callid string (should be able to store 13
 * characters or more)
 * \param buffer_size maximum writable length of the string (Less than 13 will
