...
/*! \brief Structure used to handle a large number of boolean flags == used only
   in app_dial? */
struct ast_flags64 {
  uint64_t flags;
};
 ...
 * \brief free() wrapper
 *
 * ast_free_ptr should be used when a function pointer for free() needs to be
 * passed
 ...
 *
 * ast_calloc_cache() is a wrapper for calloc() that will generate an Asterisk
 * log
 * message in the case that the allocation fails. When memory debugging is in
 * use,
 ...
 *
 * ast_vasprintf() is a wrapper for vasprintf() that will generate an Asterisk
 * log
 * message in the case that the allocation fails.
 *
 ...
 *
 * This macro will duplicate the given string.  It returns a pointer to the
 * stack
 ...
 * \return The smallest multiple of alignof(type) larger than or equal to
 * offset.
 ...
 * \retval NULL \a binary was not found or the environment variable PATH is not
 * set
 * \return \a fullpath
 */
 ...
 * \endcode
 *
 * \note This macro is especially useful for working with ao2 objects. A common
 * idiom
 * would be a function that needed to look up an ao2 object and might have
 * several error
 * conditions after the allocation that would normally need to unref the ao2
 * object.
 * With RAII_VAR, it is possible to just return and leave the cleanup to the
 * destructor
