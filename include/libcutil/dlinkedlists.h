 ...
 * \endcode
 *
 * This would define \c struct \c entry_list, and declare an instance of it
 *named
...
/*!
 * \brief Defines a structure to be used to hold a read/write list of specified
 *type.
 * \param name This will be the name of the defined structure.
 * \param type This is the type of each list entry.
 *
...
/*!
 * \brief Defines a structure to be used to hold a list of specified type (with
 *no lock).
 * \param name This will be the name of the defined structure.
 * \param type This is the type of each list entry.
 *
...
#else // if defined(AST_MUTEX_INIT_W_CONSTRUCTORS)
# define AST_DLLIST_HEAD_STATIC(name, type) \
  struct name {                             \
    struct type *first;                     \
    struct type *last;                      \
...
#endif // if defined(AST_MUTEX_INIT_W_CONSTRUCTORS)

/*!
 * \brief Defines a structure to be used to hold a read/write list of specified
 *type, statically initialized.
 * \param name This will be the name of the defined structure.
 * \param type This is the type of each list entry.
 *
 * This macro creates a structure definition that can be used
 ...
 * \param type This is the type of each list entry.
 *
 * This macro declares a structure to be used to doubly link list entries
 *together.
 ...
 * \param field This is the name of the field (declared using
 *AST_DLLIST_ENTRY())
 ...
 * this macro.
 * \param field List node field for the next node information. (declared using
 *AST_DLLIST_ENTRY())
...
#define AST_DLLIST_TRAVERSE_DIRECTION(head, var, field, start) \
  for ((var) = (head)->start; (var);                           \
       (var) = AST_DLLIST_NEXT_DIRECTION(var, field, start))
 ...
 * }
 * \endcode
 * \warning If you modify the forward-link pointer contained in the \a current
 *entry while
 * inside the loop, the behavior will be unpredictable. At a minimum, the
 *following
 * macros will modify the forward-link pointer, and should not be used inside
 * AST_DLLIST_TRAVERSE() against the entry pointed to by the \a current pointer
 *without
...
/*!
 * \brief Loops over (traverses) the entries in a list in reverse order,
 *starting at the end.
 * \param head This is a pointer to the list head structure
 * \param var This is the name of the variable that will hold a pointer to the
 * current list entry on each iteration. It must be declared before calling
 ...
 * used to link entries of this list together.
 *
 * This macro is use to loop over (traverse) the entries in a list in reverse
 *order. It uses a
 * \a for loop, and supplies the enclosed code with a pointer to each list
 * entry as it loops. It is typically used as follows:
 * \code
...
/*!
 * \brief Safe traversal of a doublly linked list using the specified direction
 *list.
 *
 * \param head List head structure pointer.
 * \param var This is the name of the variable that will hold a pointer to the
 ...
 * uses a \a for loop, and supplies the enclosed code with a pointer to each
 *list
 * node as it loops. It is typically used as follows:
 *
 * \code
 ...
 * \endcode
 *
 * It differs from AST_DLLIST_TRAVERSE() in that the code inside the loop can
 *modify
 * (or even free, after calling AST_DLLIST_REMOVE_CURRENT()) the entry pointed
 *to by
...
#define AST_DLLIST_TRAVERSE_DIRECTION_SAFE_BEGIN(head, var, field, start)          \
  do {                                                                             \
    ...
    for ((var) = __list_head->start,                                               \
         ...
         __list_next =                                                             \
           (var) ? AST_DLLIST_NEXT_DIRECTION(var, field, start) : NULL;            \
         (var);                                                                    \
...
#define AST_RWDLLIST_TRAVERSE_DIRECTION_SAFE_BEGIN \
 ...
 * used to link entries of this list together.
 *
 * \note This macro can \b only be used inside an
...
#define AST_RWDLLIST_TRAVERSE_DIRECTION_SAFE_END \
  AST_DLLIST_TRAVERSE_DIRECTION_SAFE_END
...
#define AST_RWDLLIST_TRAVERSE_BACKWARDS_SAFE_BEGIN \
  AST_DLLIST_TRAVERSE_BACKWARDS_SAFE_BEGIN

/*!
 * \brief Inserts a list entry after the current entry during a backwards
 *traversal. Since
 *        this is a backwards traversal, this will insert the entry AFTER the
 *current
 *        element. Since this is a backwards traveral, though, this would be
 *BEFORE
...
#define AST_RWDLLIST_INSERT_BEFORE_CURRENT_BACKWARDS \
...
#define AST_RWDLLIST_TRAVERSE_BACKWARDS_SAFE_END \
 ...
 * \param head This is a pointer to the list head structure
 * \param listelm This is a pointer to the entry after which the new entry
 *should
 * be inserted.
 * \param elm This is a pointer to the entry to be inserted.
 * \param field This is the name of the field (declared using
 ...
 * \param head This is a pointer to the list head structure
 * \param listelm This is a pointer to the entry before which the new entry
 *should
 * be inserted.
 * \param elm This is a pointer to the entry to be inserted.
 * \param field This is the name of the field (declared using
