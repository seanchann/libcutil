...
enum {
  ...
  /*! Don't attempt to load from realtime (typically called from a realtime
     driver dependency) */
...
enum config_save_flags {
  ...
  /*! Insure a context doesn't effectively change if a template changes (pre
     13.2 behavior) */
...
/*! \brief Structure for variables, used for configurations and for channel
   variables */
struct ast_variable {
  ...
  int inherited;                /*!< 1 for inherited from template or other
                                   base */
...
typedef struct ast_config *config_load_func (const char        *database,
                                             const char        *table,
...
typedef struct ast_variable *realtime_var_get (const char                *database,
                                               const char                *table,
                                               const struct ast_variable *fields);
typedef struct ast_config *realtime_multi_get (const char                *database,
                                               const char                *table,
                                               const struct ast_variable *fields);
typedef int realtime_update (const char *database, const char *table,
                             const char *keyfield, const char *entity,
                             const struct ast_variable *fields);
typedef int realtime_update2 (const char *database, const char *table,
                              const struct ast_variable *lookup_fields,
                              const struct ast_variable *update_fields);
typedef int realtime_store (const char *database, const char *table,
                            const struct ast_variable *fields);
typedef int realtime_destroy (const char *database, const char *table,
                              const char *keyfield, const char *entity,
                              const struct ast_variable *fields);

/*!
 * \brief Function pointer called to ensure database schema is properly
 * configured for realtime use
 * \since 1.6.1
 */
typedef int realtime_require (const char *database, const char *table,
                              va_list ap);

/*!
 * \brief Function pointer called to clear the database cache and free resources
 * used for such
 * \since 1.6.1
 */
 ...
 * CONFIG_FLAG_NOCACHE - don't cache file mtime (main purpose of this option is
 * to save memory on temporary files).
 *
 * \details
...
/*!
 * \brief Sorts categories in a config in the order of a numerical value
 ...
 * \param descending If true, we sort highest to lowest instead of lowest to
 * highest
 *
 * \details
 * This function will assume a value of 0 for any non-numerical strings and NULL
 * fields.
 */
void ast_config_sort_categories(
  struct ast_config     *config,
  int                    descending,
  int (                 *comparator)(
    struct ast_category *p,
...
/*!
 * \brief Gets the value of the LAST occurrence of a variable from a variable
 * list
 *
 ...
 *
 * \details
 * Iterates over a given ast_variable list to search for the last occurrence of
 * an
 * ast_variable entry with a name attribute matching the given name (variable).
 * This is useful if the list has duplicate entries (such as in cases where
 * entries
 ...
 * you can specify a filter to narrow the search.  The filter is a
 * comma-separated
 ...
 *
 * \details
 * This will search through the categories within a given config file for a
 * match.
 *
 ...
 * \retval 1 Always
 */
int                  ast_config_engine_register(
  struct ast_config_engine *newconfig);
 ...
 * API for backend configuration engines while building a configuration set.
 */
void                 ast_config_set_current_category(
...
struct ast_variable* _ast_variable_new(const char *name,
                                       ...
                                       const char *function,
                                       int         lineno);
# define ast_variable_new(name, value, filename) _ast_variable_new(name,                \
                                                                   value,               \
                                                                   filename,            \
                                                                   __FILE__,            \
                                                                   __PRETTY_FUNCTION__, \
 ...
 *
 * \param head A pointer to an ast_variable * of the existing variable list
 * head. May NOT be NULL
 * but the content may be to initialize a new list.  If so, upon return, this
 * parameter will be updated
 * with a pointer to the new list head.
 * \param search_hint The place in the current list to start searching for the
 * end of the list.
 ...
 *
 * \note If the existing *head is NULL, it will be updated to new_var.  This
 * allows you to call
 * ast_variable_list_append in a loop or callback without initializing the list
 * first.
 */
struct ast_variable* ast_variable_list_append_hint(struct ast_variable **head,
                                                   struct ast_variable  *search_hint,
                                                   struct ast_variable  *new_var);
#define ast_variable_list_append(head, new_var) ast_variable_list_append_hint( \
    head,                                                                      \
    NULL,                                                                      \
    new_var)
 ...
 * \param match If set, previous value of the variable (if NULL or zero-length,
...
int config_text_file_save(const char              *filename,
                          const struct ast_config *cfg,
                          const char              *generator) __attribute__((
                                                                              deprecated));
...
enum ast_parse_flags {
  ...
  PARSE_RANGE_DEFAULTS = 0x0080,       /* default to range min/max on range
                                          error */
 ...
 * returns 0, a = 223
 *     ast_parse_arg("22345", PARSE_INT32|PARSE_IN_RANGE|PARSE_DEFAULT, &a,
 * 9999, 10, 100);
 * returns 1, a = 9999
 *     ast_parse_arg("22345ssf", PARSE_UINT32|PARSE_IN_RANGE, &b, 10, 100);
 * returns 1, b unchanged
 *    ast_parse_arg("12", PARSE_UINT32|PARSE_IN_RANGE|PARSE_RANGE_DEFAULTS, &a,
 * 1, 10);
 * returns 1, a = 10
...
#define CV_STRFIELD(__x, __obj, __field) CV_F(__x,                                 \
                                              ast_string_field_set(__obj, __field, \
                                                                   __val))
 ...
 * \param right Variable to match against with an optional realtime-style
 ...
 *
 * will result in ast_strings_match("abc", "regex", "a[bdef]c") which will
 * return 1.
 *
 * left->name = "id" (ignored)
 * left->value = "abc"
 ...
 * \param right Variable list with an optional realtime-style operator in the
 * names
 * \param exact_match If true, all variables in left must match all variables in
 * right
 *        and vice versa.  This does exact value matches only.  Operators aren't
 * supported.
 *        Except for order, the left and right lists must be equal.
 *
 *        If false, every variable in the right list must match some variable in
 * the left list
 *        using the operators supplied. Variables in the left list that aren't
 * in the right
 ...
 *
 * \details
 * Iterates over the variable lists calling ast_variables_match.  If any match
 * fails
