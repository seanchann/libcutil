...
/*!
 * \brief Replace an element at a specific position in a vector, growing the
 *vector if needed.
 *
 ...
 * \return Non-zero on failure.
 *
 * \warning This macro will overwrite anything already present at the position
 *provided.
 *
 * \warning Use of this macro with the expectation that the element will remain
 *at the provided
 * index means you can not use the UNORDERED assortment of macros. These macros
 *alter the ordering
...
/*!
 * \brief Insert an element at a specific position in a vector, growing the
 *vector if needed.
 *
 ...
 * \return Non-zero on failure.
 *
 * \warning This macro will shift existing elements right to make room for the
 *new element.
 *
 * \warning Use of this macro with the expectation that the element will remain
 *at the provided
...
#define AST_VECTOR_INSERT_AT(vec, idx, elem) ({                               \
    ...
    size_t __move;                                                            \
    do {                                                                      \
      if (__make_room(((idx) > (vec)->current ? (idx) : (vec)->current),      \
                      vec) != 0) {                                            \
        res = -1;                                                             \
        break;                                                                \
      }                                                                       \
 ...
 * \return Non-zero on failure.
 *
 * \warning Use of this macro on an unsorted vector will produce unpredictable
 *results
 */
...
/*!
 * \brief Execute a callback on every element in a vector returning the first
 *matched
 *
 ...
 * or the end of the vector was reached. The vector may be empty and could be
 *NULL
 ...
 *
 * \note The type of the returned vector must be traceable to the original
 *vector.
 *
 * The following will resut in "error: assignment from incompatible pointer
 *type"
...
/*!
 * \brief Execute a callback on every element in a vector disregarding callback
 *return
 *
 * \param vec Vector to operate on.
 * \param callback A callback that takes at least 1 argument (the element)
 ...
 * \return Non-zero if error
 */
#define AST_VECTOR_RW_RDLOCK_TIMED(vec, timespec) ast_rwlock_timedrdlock( \
    &(vec)->lock,                                                         \
    timespec)
