...
/*
   ...
   Each of the reference manipulations will generate one line of output in in
      the refs
   ...
   0x86dd380,+1,1234,chan_sip.c,22242,load_module,**constructor**,allocate
      peers_by_ip
   0x822d020,+1,1234,chan_sip.c,22243,load_module,**constructor**,allocate
      dialogs
   0x8930fd8,+1,1234,chan_sip.c,20025,build_peer,**constructor**,allocate a peer
      struct
   ...
   0x89318b0,1,5678,chan_sip.c,20025,build_peer,**constructor**,allocate a peer
      struct
   ...
   0x8930218,+1,1234,chan_sip.c,20025,build_peer,**constructor**,allocate a peer
      struct
   0x8930218,+1,1234,chan_sip.c,21539,reload_config,1,link peer into peers table
   0x868c040,-1,1234,chan_sip.c,2424,dialog_unlink_all,2,unset the
      relatedpeer->call field in tandem with relatedpeer field itself
   0x868c040,-1,1234,chan_sip.c,2443,dialog_unlink_all,1,Let's unbump the count
      in the unlink so the poor pvt can disappear if it is time
   0x868c040,-1,1234,chan_sip.c,2443,dialog_unlink_all,**destructor**,Let's
      unbump the count in the unlink so the poor pvt can disappear if it is time
   0x8cc07e8,-1,1234,chan_sip.c,2370,unref_peer,3,unsetting a dialog relatedpeer
      field in sip_destroy
   0x8cc07e8,+1,1234,chan_sip.c,3876,find_peer,2,ao2_find in peers table
   0x8cc07e8,-1,1234,chan_sip.c,2370,unref_peer,3,unref_peer, from
      sip_devicestate, release ref from find_peer
   ...
 ...
 #define my_t_alloc(data,tag)  my_alloc_debug((data), tag, __FILE__, __LINE__,
   __PRETTY_FUNCTION__)
 #define my_alloc(data)        my_t_alloc((data), "")
        ...
        const char *tag, const char *file, int line, const char *func)
   {
        ...
        p = __ao2_alloc(sizeof(*p), NULL, AO2_ALLOC_OPT_LOCK_MUTEX, tag, file,
           line, func);
        if (p) {
                p->data = data;
        }
   ...
   enable refdebug in asterisk.conf.  Run asterisk, exit with "core stop
      gracefully".
   ...
   0x83787a0,+1,1234,chan_sip.c,5733,sip_alloc,**constructor**,(allocate a
      dialog(pvt) struct)
   0x83787a0,-1,1234,chan_sip.c,19173,sip_poke_peer,4,(unref dialog at end of
      sip_poke_peer, obtained from sip_alloc, just before it goes out of scope)

   0x83787a0,+1,1234,chan_sip.c,5854,sip_alloc,1,(link pvt into dialogs table)
   0x83787a0,-1,1234,chan_sip.c,19150,sip_poke_peer,3,(About to change the
      callid -- remove the old name)
   0x83787a0,+1,1234,chan_sip.c,19152,sip_poke_peer,2,(Linking in under new
      name)
   0x83787a0,-1,1234,chan_sip.c,2399,dialog_unlink_all,5,(unlinking dialog via
      ao2_unlink)

   0x83787a0,+1,1234,chan_sip.c,19130,sip_poke_peer,2,(copy sip alloc from p to
      peer->call)


   0x83787a0,+1,1234,chan_sip.c,2996,__sip_reliable_xmit,3,(__sip_reliable_xmit:
      setting pkt->owner)
   0x83787a0,-1,1234,chan_sip.c,2425,dialog_unlink_all,4,(remove all current
      packets in this dialog, and the pointer to the dialog too as part of
      __sip_destroy)

   0x83787a0,+1,1234,chan_sip.c,22356,unload_module,4,(iterate thru dialogs)
   0x83787a0,-1,1234,chan_sip.c,22359,unload_module,5,(toss dialog ptr from
      iterator_next)


   0x83787a0,+1,1234,chan_sip.c,22373,unload_module,3,(iterate thru dialogs)
   0x83787a0,-1,1234,chan_sip.c,22375,unload_module,2,(throw away iterator
      result)

   0x83787a0,+1,1234,chan_sip.c,2397,dialog_unlink_all,4,(Let's bump the count
      in the unlink so it doesn't accidentally become dead before we are done)
   0x83787a0,-1,1234,chan_sip.c,2436,dialog_unlink_all,3,(Let's unbump the count
      in the unlink so the poor pvt can disappear if it is time)

   As you can see, only one unbalanced operation is in the list, a ref count
      increment when
   ...
   Example:
        ...
        dialog_ref(dialog, "Let's bump the count in the unlink so it doesn't
           accidentally become dead before we are done");
 ...
 *//* Unlink us from the owner (channel) if we have one *//*
        if (dialog->owner) {
                 ...
                 *dialog->owner->name);
                dialog->owner->tech_pvt = dialog_unref(dialog->owner->tech_pvt,
        ...
        if (dialog->registry) {
                if (dialog->registry->call == dialog) {
                        dialog->registry->call =
                         *dialog_unref(dialog->registry->call, "nulling out the
                         *registry's call dialog field in unlink_all");
                }
                dialog->registry = registry_unref(dialog->registry, "delete
                 *dialog->registry");
        }
        ...
        dialog_unref(dialog, "Let's unbump the count in the unlink so the poor
         *pvt can disappear if it is time");
...
#define ao2_t_ref(o, delta, tag) __ao2_ref((o),      \
                                           (delta),  \
                                           (tag),    \
                                           __FILE__, \
                                           __LINE__, \
                                           __PRETTY_FUNCTION__)
#define ao2_ref(o, delta) __ao2_ref((o),      \
                                    (delta),  \
                                    "",       \
                                    __FILE__, \
                                    __LINE__, \
...
typedef void (*ao2_weakproxy_notification_cb)(void *weakproxy,
                                              void *data);
...
#define ao2_weakproxy_alloc(data_size, destructor_fn) \
  __ao2_weakproxy_alloc(data_size,                    \
                        destructor_fn,                \
                        "",                           \
                        __FILE__,                     \
                        __LINE__,                     \
...
#define ao2_weakproxy_set_object(weakproxy, obj, flags) \
  __ao2_weakproxy_set_object(weakproxy,                 \
                             obj,                       \
                             flags,                     \
                             "",                        \
                             __FILE__,                  \
...
#define ao2_weakproxy_get_object(weakproxy, flags) \
  __ao2_weakproxy_get_object(weakproxy,            \
                             flags,                \
                             "",                   \
                             __FILE__,             \
                             __LINE__,             \
 ...
 *
 * \note If the weakproxy currently points to NULL the callback will be run
 *immediately,
 ...
 *       unsubscribed as many times.  The OBJ_MULTIPLE flag can be used to
 *remove
...
int __ao2_lock(void             *a,
               ...
               int               line,
               const char       *var);
#define ao2_lock(a) __ao2_lock(a,                   \
                               AO2_LOCK_REQ_MUTEX,  \
                               __FILE__,            \
                               __PRETTY_FUNCTION__, \
                               __LINE__,            \
                               # a)
#define ao2_rdlock(a) __ao2_lock(a,                   \
                                 AO2_LOCK_REQ_RDLOCK, \
                                 __FILE__,            \
                                 __PRETTY_FUNCTION__, \
                                 __LINE__,            \
                                 # a)
#define ao2_wrlock(a) __ao2_lock(a,                   \
                                 AO2_LOCK_REQ_WRLOCK, \
                                 __FILE__,            \
                                 __PRETTY_FUNCTION__, \
                                 __LINE__,            \
...
int __ao2_unlock(void       *a,
                 ...
                 int         line,
                 const char *var);
#define ao2_unlock(a) __ao2_unlock(a, __FILE__, __PRETTY_FUNCTION__, __LINE__, \
                                   # a)
...
int __ao2_trylock(void             *a,
                  ...
                  int               line,
                  const char       *var);
#define ao2_trylock(a) __ao2_trylock(a,                   \
                                     AO2_LOCK_REQ_MUTEX,  \
                                     __FILE__,            \
                                     __PRETTY_FUNCTION__, \
                                     __LINE__,            \
                                     # a)
#define ao2_tryrdlock(a) __ao2_trylock(a,                   \
                                       AO2_LOCK_REQ_RDLOCK, \
                                       __FILE__,            \
                                       __PRETTY_FUNCTION__, \
                                       __LINE__,            \
                                       # a)
#define ao2_trywrlock(a) __ao2_trylock(a,                   \
                                       AO2_LOCK_REQ_WRLOCK, \
                                       __FILE__,            \
                                       __PRETTY_FUNCTION__, \
                                       __LINE__,            \
...
/*!
 * \brief Define a global object holder to be used to hold an ao2 object,
...
#else // ifndef HAVE_PTHREAD_RWLOCK_INITIALIZER
# define AO2_GLOBAL_OBJ_STATIC(name) \
  struct ao2_global_obj name = {     \
    .lock = AST_RWLOCK_INIT_VALUE,   \
  }
#endif // ifndef HAVE_PTHREAD_RWLOCK_INITIALIZER
...
#define ao2_t_global_obj_release(holder, tag)   \
  __ao2_global_obj_release(&holder,             \
                           (tag),               \
                           __FILE__,            \
                           __LINE__,            \
                           __PRETTY_FUNCTION__, \
...
#define ao2_t_global_obj_replace(holder, obj, tag) \
  __ao2_global_obj_replace(& holder,               \
                           (obj),                  \
                           (tag),                  \
                           __FILE__,               \
                           __LINE__,               \
...
/*!
 * \brief Replace an ao2 object in the global holder, throwing away any old
 *object.
 * \since 11.0
 *
 * \param holder Global ao2 object holder.
...
#define ao2_t_global_obj_replace_unref(holder, obj, tag) \
  __ao2_global_obj_replace_unref(&holder,                \
                                 (obj),                  \
                                 (tag),                  \
                                 __FILE__,               \
                                 __LINE__,               \
...
/*!
   ...
   -  \b ao2_find(c, arg, flags)
    ...
    OBJ_SEARCH_PARTIAL_KEY - if set, 'arg', is a partial search key item that is
       not an object.
   ...
   -  \b ao2_callback(c, flags, fn, arg)
    ...
    do anything with the object including unlinking it.
      ...
      - flags can be
         ...
         OBJ_NODATA   - don't return the object if found (no ref count change)
         OBJ_MULTIPLE - don't stop at first match
         OBJ_SEARCH_OBJECT - if set, 'arg' is an object pointer, and a hash
            table
                        search will be done. If not, a traversal is done through
                        all the hash table 'buckets'..
         OBJ_SEARCH_KEY - if set, 'arg', is a search key item that is not an
            object.
                        Similar to OBJ_SEARCH_OBJECT and mutually exclusive.
         OBJ_SEARCH_PARTIAL_KEY - if set, 'arg', is a partial search key item
...
/*!
 * \brief Flags passed to ao2_callback_fn(), ao2_hash_fn(), and ao2_sort_fn() to
 *modify behaviour.
 */
enum search_flags {
  /*!
 ...
 *   OBJ_SEARCH_KEY - if set, 'arg', is a search key item that is not an object.
 *   OBJ_SEARCH_PARTIAL_KEY - if set, 'arg', is a partial search key item that
 *is not an object.
 *
 ...
 */
typedef int (ao2_callback_fn)(void *obj,
                              void *arg,
                              int   flags);

/*! \brief A common ao2_callback is one that matches by address. */
int ao2_match_by_addr(void *obj,
                      void *arg,
                      int   flags);
 ...
 * Callback functions are used to search or manipulate objects in a container.
 */
typedef int (ao2_callback_data_fn)(void *obj,
                                   void *arg,
                                   void *data,
                                   int   flags);
 ...
 */
typedef int (ao2_hash_fn)(const void *obj,
                          int         flags);
 ...
 *   OBJ_SEARCH_KEY - if set, 'obj_right', is a search key item that is not an
 *object.
 *   OBJ_SEARCH_PARTIAL_KEY - if set, 'obj_right', is a partial search key item
 *that is not an object.
 *
 * \note This function must be idempotent.
 *
 ...
 * \retval >0 if obj_left > obj_right
 */
typedef int (ao2_sort_fn)(const void *obj_left,
...
/*!
 * \brief Allocate and initialize a hash container with the desired number of
 *buckets.
 *
 * \details
 ...
 * \param hash_fn Pointer to a function computing a hash value. (NULL if
 *everyting goes in first bucket.)
 * \param cmp_fn Pointer to a compare function used by ao2_find. (NULL to match
 *everything)
 * \param tag used for debugging.
 *
...
#define ao2_t_container_alloc_options(options, n_buckets, hash_fn, cmp_fn, tag) \
  ao2_t_container_alloc_hash((options),                                         \
                             0,                                                 \
                             (n_buckets),                                       \
                             (hash_fn),                                         \
                             NULL,                                              \
...
#define ao2_t_container_alloc(n_buckets, hash_fn, cmp_fn, tag) \
  ao2_t_container_alloc_options(AO2_ALLOC_OPT_LOCK_MUTEX,      \
                                (n_buckets),                   \
                                (hash_fn),                     \
                                (cmp_fn),                      \
                                (tag))
#define ao2_container_alloc(n_buckets, hash_fn, cmp_fn)                         \
  ao2_container_alloc_options(AO2_ALLOC_OPT_LOCK_MUTEX, (n_buckets), (hash_fn), \
                              (cmp_fn))
 ...
 * \param container_options Container behaviour options (See enum
 *ao2_container_opts)
 * \param n_buckets Number of buckets for hash
 * \param hash_fn Pointer to a function computing a hash value. (NULL if
 *everyting goes in first bucket.)
...
#define ao2_t_container_alloc_hash(ao2_options,       \
                                   ...
                                   cmp_fn,            \
                                   tag)               \
  __ao2_container_alloc_hash((ao2_options),           \
                             (container_options),     \
                             (n_buckets),             \
                             (hash_fn),               \
                             (sort_fn),               \
...
#define ao2_t_container_alloc_list(ao2_options,       \
                                   ...
                                   cmp_fn,            \
                                   tag)               \
  __ao2_container_alloc_list((ao2_options),           \
                             (container_options),     \
                             (sort_fn),               \
                             (cmp_fn),                \
                             (tag),                   \
...
#define ao2_t_container_alloc_rbtree(ao2_options,       \
                                     ...
                                     cmp_fn,            \
                                     tag)               \
  __ao2_container_alloc_rbtree((ao2_options),           \
                               (container_options),     \
                               (sort_fn),               \
                               (cmp_fn),                \
                               (tag),                   \
                               ...
                               __LINE__,                \
                               __PRETTY_FUNCTION__)
#define ao2_container_alloc_rbtree(ao2_options, container_options, sort_fn, \
                                   cmp_fn)                                  \
  __ao2_container_alloc_rbtree((ao2_options),                               \
                               (container_options),                         \
                               (sort_fn),                                   \
...
/*!
 * \brief Copy all object references in the src container into the dest
 *container.
 * \since 11.0
 *
 ...
 * \return Nothing
 */
typedef void (ao2_prnt_obj_fn)(void        *v_obj,
                               void        *where,
                               ao2_prnt_fn *prnt);
...
#define ao2_t_link(container, obj, tag)                        \
  __ao2_link((container), (obj), 0, (tag), __FILE__, __LINE__, \
             __PRETTY_FUNCTION__)
#define ao2_link(container, obj)                               \
  __ao2_link((container), (obj), 0, "",    __FILE__, __LINE__, \
             __PRETTY_FUNCTION__)
 ...
 * \retval NULL, always
 *
 * \note The object requested to be unlinked must be valid.  However, if it
 *turns
 ...
 * instead of taking an ao2_callback_fn *, it takes an ao2_callback_data_fn *,
 *and
 * allows the caller to pass in arbitrary data.
 *
 * This call would be used instead of ao2_callback() when the caller needs to
 *pass
 * OBJ_SEARCH_OBJECT, OBJ_SEARCH_KEY, or OBJ_SEARCH_PARTIAL_KEY as part of the
 *flags
 * argument (which in turn requires passing in a known pointer type for 'arg')
 *and
 * also needs access to other non-global data to complete it's comparison or
 *task.
 *
...
#else // if defined(TEST_FRAMEWORK)
...
void __ao2_cleanup_debug(void       *obj,
                         ...
                         int         line,
                         const char *function);
#define ao2_cleanup(obj) __ao2_cleanup_debug((obj),    \
                                             "",       \
                                             __FILE__, \
                                             __LINE__, \
                                             __PRETTY_FUNCTION__)
#define ao2_t_cleanup(obj, tag) __ao2_cleanup_debug((obj),    \
                                                    (tag),    \
                                                    __FILE__, \
                                                    __LINE__, \
                                                    __PRETTY_FUNCTION__)
