...
struct ast_threadstorage {
  pthread_once_t once;                   /*!< Ensure that the key is only
                                            initialized by one thread */
  pthread_key_t  key;                    /*!< The key used to retrieve this
                                            thread's data */
  void           (*key_init)(void);      /*!< The function that initializes the
                                            key */
  int            (*custom_init)(void *); /*!< Custom initialization function
                                            specific to the object */
};

#if defined(DEBUG_THREADLOCALS)
void __ast_threadstorage_object_add(void        *key,
                                    size_t       len,
                                    const char  *file,
                                    const char  *function,
                                    unsigned int line);
void __ast_threadstorage_object_remove(void *key);
void __ast_threadstorage_object_replace(void  *key_old,
                                        void  *key_new,
                                        size_t len);
...
/*!
 * \brief Define a thread storage variable, with custom initialization and
 *cleanup
 *
 * \param a The name of the thread storage object
 * \param b This is a custom function that will be called after each thread
 *specific
 ...
 * \endcode
 */
#define AST_THREADSTORAGE_CUSTOM(a, b, c) AST_THREADSTORAGE_CUSTOM_SCOPE(a, \
                                                                         b, \
                                                                         c, \
                                                                         static)
...
#else // if defined(PTHREAD_ONCE_INIT_NEEDS_BRACES)
# define AST_PTHREAD_ONCE_INIT PTHREAD_ONCE_INIT
#endif // if defined(PTHREAD_ONCE_INIT_NEEDS_BRACES)
...
#else /* defined(DEBUG_THREADLOCALS) */
AST_INLINE_API(
  void* __ast_threadstorage_get(struct ast_threadstorage *ts,
                                size_t                    init_size,
                                const char               *file,
                                const char               *function,
                                unsigned int              line),
...
# define ast_threadstorage_get(ts, init_size) __ast_threadstorage_get(ts,                  \
                                                                      init_size,           \
                                                                      __FILE__,            \
                                                                      __PRETTY_FUNCTION__, \
                                                                      __LINE__)
