 ...
 */
typedef void *(*aco_type_item_find)(struct ao2_container *newcontainer,
                                    const char *category);

/*! \brief Callback function that is called after a config object is initialized
 * with defaults
 *
 * \note This callback is called during config processing after a new config is
 * allocated and
 * and defaults applied but before values from the config are read. This
 * callback could be used
 * to merge in settings inherited from the global settings if necessary, despite
 * that being a
 * bad thing to do!
 *
...
/*! \brief Callback function that is called after config processing, but before
 * linking
 *
 * \note This callback is called after config processing, but before linking the
 * object
 * in the config container. This callback can be used to verify that all
 * settings make
...
/*! \brief A function for determining whether the value for the matchfield in an
 * aco_type is sufficient for a match
...
struct aco_type {
  ...
  const char *category; /*!< A regular expression for matching categories to be
                           allowed or denied */
  const char *matchfield; /*!< An option name to match for this type (i.e. a
                             'type'-like column) */
  const char *matchvalue; /*!< The value of the option to require for matching
                             (i.e. 'peer' for type= in sip.conf) */
  aco_matchvalue_func matchfunc; /*!< A function for determing whether the
                                    option value matches (i.e. hassip= requires
                                    ast_true()) */
  enum aco_category_op
      category_match;  /*!< Whether the following category regex is a whitelist
                          or blacklist */
  size_t item_offset;  /*!< The offset in the config snapshot for the global
                          config or item config container */
  unsigned int hidden; /*!< Type is for internal purposes only and it and all
                          options should not be visible to users */

  /* non-global callbacks */
  aco_type_item_alloc item_alloc; /*!< An allocation function for item
                                     associated with this type */
  aco_type_item_find item_find;   /*!< A callback function to find an existing
                                     item in a particular container */
  aco_type_item_pre_process
      item_pre_process; /*!< An optional callback function that is called after
                           defaults are applied, but before config processing */
  aco_type_prelink item_prelink; /*!< An optional callback function that is
                                    called after config processing, but before
                                    applying changes */
...
/*! \brief A callback function for allocating an object to hold all config
 * objects
 * \retval NULL error
...
struct aco_file {
  ...
  const char *
      alias; /*!< An alias filename to be tried if 'filename' cannot be found */
  const char *
      *preload; /*!< A null-terminated ordered array of categories to be loaded
                   first */
  const char
      *skip_category; /*!< A regular expression of categories to skip in the
                         file. Use when a file is processed by multiple modules
                         */
  struct aco_type *types[]; /*!< The list of types for this config. Required.
                               Use a sentinel! */
};

struct aco_info {
  const char
      *module;    /*!< The name of the module whose config is being processed */
  int hidden : 1; /*!< If enabled, this config item is hidden from users */
  aco_pre_apply_config pre_apply_config; /*!< A callback called after
                                            processing, but before changes are
                                            applied */
  aco_post_apply_config
      post_apply_config; /*!< A callback called after changes are applied */
  aco_snapshot_alloc
      snapshot_alloc; /*!< Allocate an object to hold all global configs and
                         item containers */
  struct ao2_global_obj
      *global_obj; /*!< The global object array that holds the user-defined
                      config object */
 ...
 * \note This will most likely be called from the pre_apply_config callback
 * function
 * \param info An initialized aco_info
 * \retval NULL error
 * \retval non-NULL A pointer to the user-defined config object with un-applied
 * changes
 */
...
#define CONFIG_INFO_STANDARD(name, arr, alloc, ...)       \
  static struct aco_info name = {.module = CUTIL_MODULE,  \
                                 .global_obj = &arr,      \
                                 .snapshot_alloc = alloc, \
                                 __VA_ARGS__};

#define CONFIG_INFO_CORE(mod, name, arr, alloc, ...)      \
  static struct aco_info name = {.module = mod,           \
                                 .global_obj = &arr,      \
                                 .snapshot_alloc = alloc, \
                                 __VA_ARGS__};
...
enum aco_option_type {
   ...
   *     struct ast_ha *ha;
   * };
   * aco_option_register(&cfg_info, "permit", ACO_EXACT, my_types, NULL,
   * OPT_ACL_T, 1, FLDSET(struct test_item, ha));
   * aco_option_register(&cfg_info, "deny", ACO_EXACT, my_types, NULL,
   * OPT_ACL_T, 0, FLDSET(struct test_item, ha));
   * {code}
   */
  OPT_ACL_T,
   ...
   * aco_option_register varargs:
   *   FLDSET macro with the field of type int. It is important to note that the
   * field
   *   cannot be a bitfield. If bitfields are required, they must be set via a
   * custom handler.
   *
   * Example:
   * {code}
  ...
  /*! \brief Type for default option handler for bools (ast_true/ast_false) that
   * are stored in a flag
   * \note aco_option_register flags:
   *   non-zero : process via ast_true
   *   0        : process via ast_false
   * aco_option_register varargs:
   ...
   * };
   * aco_option_register(&cfg_info, "description", ACO_EXACT, my_types, "none",
   * OPT_CHAR_ARRAY_T, 0, CHARFLDSET(struct test_item, description));
   * {endcode}
   */
  OPT_CHAR_ARRAY_T,
   ...
   * };
   * aco_option_register(&cfg_info, "allow", ACO_EXACT, my_types, "ulaw,alaw",
   * OPT_CODEC_T, 1, FLDSET(struct test_item, cap));
   * aco_option_register(&cfg_info, "disallow", ACO_EXACT, my_types, "all",
   * OPT_CODEC_T, 0, FLDSET(struct test_item, cap));
   * {endcode}
   */
  OPT_CODEC_T,
   ...
   *
   * \note aco_option_register flags:
   *   See flags available for use with the PARSE_DOUBLE type for the
   * ast_parse_arg function
   * aco_option_register varargs:
   ...
   * };
   * {code}
   * aco_option_register(&cfg_info, "doubleopt", ACO_EXACT, my_types, "3",
   * OPT_DOUBLE_T, 0, FLDSET(struct test_item, dub));
   * {endcode}
   */
  OPT_DOUBLE_T,
   ...
   *
   * \note aco_option_register flags:
   *   See flags available for use with the PARSE_INT32 type for the
   * ast_parse_arg function
   * aco_option_register varargs:
   *   FLDSET macro with the field of type int32_t
   *   The remaining varargs for should be arguments compatible with the varargs
   * for the
   *   ast_parse_arg function with the PARSE_INT32 type and the flags passed in
   * the
   *   aco_option_register flags parameter.
   *
   * \note In most situations, it is preferable to not pass the PARSE_DEFAULT
   * flag. If a config
   * contains an invalid value, it is better to let the config loading fail with
   * warnings so that
   ...
   * };
   * {code}
   * aco_option_register(&cfg_info, "intopt", ACO_EXACT, my_types, "3",
   * OPT_INT_T, PARSE_IN_RANGE, FLDSET(struct test_item, intopt), -10, 10);
   * {endcode}
   */
  OPT_INT_T,
   ...
   *
   * \note aco_option_register flags:
   *   See flags available for use with the PARSE_ADDR type for the
   * ast_parse_arg function
   * aco_option_register varargs:
   ...
   * };
   * aco_option_register(&cfg_info, "sockaddropt", ACO_EXACT, my_types,
   * "0.0.0.0:1234", OPT_SOCKADDR_T, 0, FLDSET(struct test_item, addr));
   * {endcode}
   */
  OPT_SOCKADDR_T,
   ...
   *     );
   * };
   * aco_option_register(&cfg_info, "thing", ACO_EXACT, my_types, NULL,
   * OPT_STRINGFIELD_T, 0, STRFLDSET(struct test_item, thing));
   * {endcode}
   */
  OPT_STRINGFIELD_T,
   ...
   *
   * \note aco_option_register flags:
   *   See flags available for use with the PARSE_UINT32 type for the
   * ast_parse_arg function
   * aco_option_register varargs:
   *   FLDSET macro with the field of type uint32_t
   *   The remaining varargs for should be arguments compatible with the varargs
   * for the
   *   ast_parse_arg function with the PARSE_UINT32 type and the flags passed in
   * the
   *   aco_option_register flags parameter.
   *
   * \note In most situations, it is preferable to not pass the PARSE_DEFAULT
   ...
   * };
   * {code}
   * aco_option_register(&cfg_info, "uintopt", ACO_EXACT, my_types, "3",
   * OPT_UINT_T, PARSE_IN_RANGE, FLDSET(struct test_item, uintopt), 1, 10);
   * {endcode}
   */
  OPT_UINT_T,
   ...
   * translated to a string, OPT_BOOL_T becomes "true" or "false"; OPT_YESNO_T
   * becomes
   * "yes" or "no".
   *
   * Example:
   ...
   * };
   * aco_option_register(&cfg_info, "enabled", ACO_EXACT, my_types, "no",
   * OPT_YESNO_T, 1, FLDSET(struct test_item, enabled));
   * {endcode}
   */
  OPT_YESNO_T,
...
enum aco_process_status {
  ...
  ACO_PROCESS_UNCHANGED, /*!< \brief The config had not been edited and no
                            changes applied */
...
/*! \brief Process config info from an ast_config via options registered with an
 * aco_info
 *
 ...
 * \note This function can be used to build up an object by repeatedly passing
 * in
 * the config variable name and values that would be found in a config file.
 * This can
 * be useful if the object is to be populated by a dialplan function, for
 * example.
 *
 ...
 * \retval -1 Failure
 */
int aco_process_var(struct aco_type *type, const char *cat,
                    struct ast_variable *var, void *obj);
 ...
 * \param obj The user-defined config object that will store the parsed config
 * items
 *
 * \retval 0 Success
 * \retval -1 Failure
 ...
 * \param category The configuration category from which \a obj is being
 * configured
 ...
 *
 * \note this should probably only be called by one of the aco_option_register*
 * macros
 *
 * \param info The aco_info holding this module's config information
 * \param name The name of the option
 * \param match_type
 * \param types An array of valid option types for matching categories to the
 * correct struct type
 * \param default_val The default value of the option in the same format as
 * defined in a config file
 * \param type The option type (only for default handlers)
 * \param handler The handler function for the option (only for non-default
 * types)
 * \param flags a type specific flags, stored in the option and available to the
 * handler
...
int __aco_option_register(struct aco_info *info, const char *name,
                          ...
                          struct aco_type **types, const char *default_val,
                          enum aco_option_type type, aco_option_handler handler,
                          unsigned int flags, unsigned int no_doc, size_t argc,
                          ...);
 ...
 * \retval -1 Failure
 */
#define aco_option_register(info, name, matchtype, types, default_val,       \
                            opt_type, flags, ...)                            \
  __aco_option_register(info, name, matchtype, types, default_val, opt_type, \
                        NULL, flags, 0, VA_NARGS(__VA_ARGS__), __VA_ARGS__);
 ...
 * \param flags \a type specific flags, stored in the option and available to
 * the handler
 *
 * \retval 0 Success
 * \retval -1 Failure
 */
#define aco_option_register_custom(info, name, matchtype, types, default_val, \
                                   handler, flags)                            \
  __aco_option_register(info, name, matchtype, types, default_val,            \
                        OPT_CUSTOM_T, handler, flags, 0, 0);
 ...
 * the handler
 *
 * \note This is used primarily with custom options that only have internal
 * purposes
 ...
 * \retval -1 Failure
 */
#define aco_option_register_custom_nodoc(info, name, matchtype, types, \
                                         default_val, handler, flags)  \
  __aco_option_register(info, name, matchtype, types, default_val,     \
                        OPT_CUSTOM_T, handler, flags, 1, 0);
 ...
 * \param types An array of valid option types for matching categories to the
 * correct struct type
 * \param aliased_to The name of the option that this deprecated option matches
 * to
 *
 * \retval 0 Success
 * \retval -1 Failure
 */
int aco_option_register_deprecated(struct aco_info *info, const char *name,
                                   struct aco_type **types,
                                   const char *aliased_to);

/*!
 * \brief Read the flags of a config option - useful when using a custom
 * callback for a config option
 * \since 12
 *
 ...
 * \param func_arg The first argument (almost certainly a type (e.g. "struct
 * mystruct")
 ...
 * ARGMAP_1(offsetof, struct foo, ARGIFY(3, offsetof(struct foo, a),
 * offsetof(struct foo, b)), c) ->
 * ARGIFY(3, offsetof(struct foo, a), offsetof(struct foo, b), offsetof(struct
 * foo, c)) ->
...
/*! \note This is sneaky. On the very first argument, we set "in" to N, the
 * number of arguments, so
 * that the accumulation both works properly for the first argument (since "in"
 * can't be empty) and
 * we get the number of arguments in our varargs as a bonus
 */
#define ARGMAP_(N, func, func_arg, x, ...) \
  PASTE(ARGMAP_, N)(func, func_arg, N, x, __VA_ARGS__)

/*! \def PASTE(arg1, arg2)
 * \brief Paste two arguments together, even if they are macros themselves
 * \note Uses two levels to handle the case where arg1 and arg2 are macros
 * themselves
 */
#define PASTE(arg1, arg2) PASTE1(arg1, arg2)
#define PASTE1(arg1, arg2) arg1##arg2

/*! \brief Take a comma-separated list and allow it to be passed as a single
 ...
 * \param func_arg The first argument to func (most likely a type e.g. "struct
 * my_struct")
 ...
 * \note Currently only up to 8, but expanding is easy. This macro basically
 * counts
 ...
 *
 * Note that VA_NARGS *does not* work when there are no arguments passed.
 * Pasting an empty
 * __VA_ARGS__ with a comma like ", ##__VA_ARGS__" will delete the leading
 * comma, but it
 * does not work when __VA_ARGS__ is the first argument. Instead, 1 is returned
 * instead of 0:
 *
 * VA_NARGS() ->                              v
...
/*! \def FLDSET(type, ...)
 * \brief Convert a struct and list of fields to an argument list of field
 * offsets
 * \param type The type with the fields (e.g. "struct my_struct")
 * \param ... varags The fields in the struct whose offsets are needed as
 * arguments
 *
 * For example:
 * \code
...
/*! \def STRFLDSET(type, ...)
 * \brief Convert a struct and a list of stringfield fields to an argument list
 * of field offsets
 * \note Stringfields require the passing of the field manager pool, and field
 * manager to the
 * default stringfield option handler, so registering options that point to
 * stringfields requires
 * this macro to be called instead of the FLDSET macro.
 * \param type The type with the fields (e.g. "struct my_struct")
 * \param ... varargs The fields in the struct whose offsets are needed as
 * arguments
 */
 ...
 * \brief A helper macro to pass the appropriate arguments to
 * aco_option_register for OPT_CHAR_ARRAY_T
 ...
 * \note Used internally to remove the leading "number of arguments" argument
 * from ARGMAP for
 * FLDSET. This is because a call to FLDSET may be followed by additional
 * arguments in
 * aco_register_option, so the true number of arguments will possibly be
 * different than what
 * ARGMAP returns.
