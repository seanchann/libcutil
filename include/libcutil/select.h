...
#else // if !defined(HAVE_VARIABLE_FDSET) && defined(CONFIGURE_RAN_AS_ROOT)
# define ast_FDMAX       32768
typedef struct {
  TYPEOF_FD_SET_FDS_BITS fds_bits[ast_FDMAX / 8 / SIZEOF_FD_SET_FDS_BITS]; /*
                                                                              32768
                                                                              bits
                                                                              */
} ast_fdset;
...
# define FD_SET(fd, fds)                                                     \
  do {                                                                       \
    ...
    if (fd < ast_FDMAX) {                                                    \
      bytes[fd / _bitsize(*bytes)] |= ((TYPEOF_FD_SET_FDS_BITS)1) <<         \
                                      (fd % _bitsize(*bytes));               \
    } else {                                                                 \
 ...
 */
static inline int ast_select(int             nfds,
                             ast_fdset      *rfds,
                             ast_fdset      *wfds,
                             ast_fdset      *efds,
                             struct timeval *tvp)
{
  ...
  if (tvp) {
    ...
    res =
      select(nfds, (fd_set *)rfds, (fd_set *)wfds, (fd_set *)efds, tvp);
    save_errno = errno;
  ...
  else return select(nfds, (fd_set *)rfds, (fd_set *)wfds, (fd_set *)efds, NULL);
