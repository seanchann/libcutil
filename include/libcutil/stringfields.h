...
/*!
   ...
   \param field_list The list of fields to declare, using AST_STRING_FIELD() for
      each one.
...
#define ast_string_field_free_memory(x)                                           \
  ({                                                                              \
    ...
    if (((void *)(x)) != NULL) {                                                  \
      __res__ =                                                                   \
        __ast_string_field_free_memory(&(x)->__field_mgr, &(x)->__field_mgr_pool, \
                                       AST_STRINGFIELD_DESTROY, __FILE__,         \
...
#define ast_calloc_with_stringfields(n, type, size)                                    \
  __ast_calloc_with_stringfields(n, sizeof(type), offsetof(type, __field_mgr),         \
                                 offsetof(type,                                        \
                                          __field_mgr_pool), size, __FILE__, __LINE__, \
                                 __PRETTY_FUNCTION__)
...
/*!
   \brief Macro to provide access to the allocation field that lives immediately
      in front of a string field
   \param x Pointer to the string field

   Note that x must be a pointer to a byte-sized type -- normally (char *) -- or
      this calculation
   would break horribly
 */
#define AST_STRING_FIELD_ALLOCATION(x) *((ast_string_field_allocation *)(x -          \
                                                                         __alignof__( \
                                                                           ast_string_field_allocation)))
...
#define ast_string_field_ptr_set(x, ptr, data)                    \
  ({                                                              \
    ...
    if (((void *)(x)) != NULL) {                                  \
      __res__ =                                                   \
        ast_string_field_ptr_set_by_fields((x)->__field_mgr_pool, \
                                           (x)->__field_mgr,      \
                                           ptr, data);            \
    }                                                             \
    __res__;                                                      \
    ...
    if (__dlen__ == 1) {                                                         \
      ...
      *__p__ = __ast_string_field_empty;                                         \
               ...
               (!__ast_string_field_ptr_grow(&field_mgr, &field_mgr_pool,        \
                                             __dlen__, __p__)) ||                \
               (target =                                                         \
                  __ast_string_field_alloc_space(&field_mgr, &field_mgr_pool,    \
                                                 __dlen__))) {                   \
      if (target != *__p__) {                                                    \
        __ast_string_field_release_active(field_mgr_pool, *__p__);               \
        *__p__ = target;                                                         \
...
#define ast_string_field_ptr_build(x, ptr, fmt, args ...)                     \
  ({                                                                          \
    int __res__ = -1;                                                         \
    if (((void *)(x)) != NULL) {                                              \
      __ast_string_field_ptr_build(&(x)->__field_mgr, &(x)->__field_mgr_pool, \
                                   (ast_string_field *)ptr, fmt, args);       \
      __res__ = 0;                                                            \
    }                                                                         \
    __res__;                                                                  \
  })
...
#define ast_string_field_build(x, field, fmt, args ...)                         \
  ({                                                                            \
    ...
    if (((void *)(x)) != NULL) {                                                \
      __ast_string_field_ptr_build(&(x)->__field_mgr, &(x)->__field_mgr_pool,   \
                                   (ast_string_field *)&(x)->field, fmt, args); \
      __res__ = 0;                                                              \
    }                                                                           \
    __res__;                                                                    \
  })
...
#define ast_string_field_ptr_build_va(x, ptr, fmt, args)                         \
  ({                                                                             \
    int __res__ = -1;                                                            \
    if (((void *)(x)) != NULL) {                                                 \
      __ast_string_field_ptr_build_va(&(x)->__field_mgr, &(x)->__field_mgr_pool, \
                                      (ast_string_field *)ptr, fmt, args);       \
      __res__ = 0;                                                               \
    }                                                                            \
    __res__;                                                                     \
...
/*!
   ...
   \retval zero if all string fields are equal (does not compare non-string
      field data)
...
/*!
   \brief Copy all string fields from one instance to another of the same
      structure
   \since 12
  ...
  ({                                                                                     \
    ...
    if (((void *)(copy)) != NULL && ((void *)(orig)) != NULL) {                          \
      __res__ = __ast_string_fields_copy(((copy)->__field_mgr_pool),                     \
                                         (struct ast_string_field_mgr *)&((copy)->       \
                                                                          __field_mgr),  \
                                         (struct ast_string_field_mgr *)&((orig)->       \
                                                                          __field_mgr)); \
    }                                                                                    \
    __res__;                                                                             \
  })
