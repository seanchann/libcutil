...
static struct ast_uri *ast_uri_create_(
    const char *scheme, unsigned int scheme_size, const char *user_info,
    unsigned int user_info_size, const char *host, unsigned int host_size,
    const char *port, unsigned int port_size, const char *path,
    unsigned int path_size, const char *query, unsigned int query_size) {
#define SET_VALUE(param, field, size)  \
  ...
  char *p;
  struct ast_uri *res =
      ao2_alloc(sizeof(*res) + scheme_size + user_info_size + host_size +
...
struct ast_uri *ast_uri_create(const char *scheme, const char *user_info,
  ...
  return ast_uri_create_(
      scheme, scheme ? strlen(scheme) + 1 : 0, user_info,
      user_info ? strlen(user_info) + 1 : 0, host, host ? strlen(host) + 1 : 0,
      port, port ? strlen(port) + 1 : 0, path, path ? strlen(path) + 1 : 0,
      query, query ? strlen(query) + 1 : 0);
}

struct ast_uri *ast_uri_copy_replace(const struct ast_uri *uri,
                                     const char *scheme, const char *user_info,
                                     const char *host, const char *port,
                                     const char *path, const char *query) {
  return ast_uri_create(scheme ? scheme : uri->scheme,
                        user_info ? user_info : uri->user_info,
                        host ? host : uri->host, port ? port : uri->port,
                        path ? path : uri->path, query ? query : uri->query);
}

const char *ast_uri_scheme(const struct ast_uri *uri) { return uri->scheme; }

const char *ast_uri_user_info(const struct ast_uri *uri) {
  return uri->user_info;
}

const char *ast_uri_host(const struct ast_uri *uri) { return uri->host; }

const char *ast_uri_port(const struct ast_uri *uri) { return uri->port; }

const char *ast_uri_path(const struct ast_uri *uri) { return uri->path; }

const char *ast_uri_query(const struct ast_uri *uri) { return uri->query; }

int ast_uri_is_secure(const struct ast_uri *uri) {
  return ast_strlen_zero(uri->scheme)
             ? 0
...
struct ast_uri *ast_uri_parse(const char *uri) {
  ...
  scheme_size =
      uria.scheme.first ? uria.scheme.afterLast - uria.scheme.first + 1 : 0;
  user_info_size = uria.userInfo.first
                       ? uria.userInfo.afterLast - uria.userInfo.first + 1
                       : 0;
  host_size = uria.hostText.first
                  ? uria.hostText.afterLast - uria.hostText.first + 1
                  : 0;
  port_size = uria.portText.first
                  ? uria.portText.afterLast - uria.portText.first + 1
                  : 0;

  path_start = uria.pathHead && uria.pathHead->text.first
                   ? uria.pathHead->text.first
                   : NULL;
  path_end = path_start ? uria.pathTail->text.afterLast : NULL;
  path_size = path_end ? path_end - path_start + 1 : 0;

  query_size =
      uria.query.first ? uria.query.afterLast - uria.query.first + 1 : 0;

  res = ast_uri_create_(uria.scheme.first, scheme_size, uria.userInfo.first,
                        user_info_size, uria.hostText.first, host_size,
                        uria.portText.first, port_size, path_start, path_size,
...
#define SET_VALUES(value)     \
  ...
  return ast_uri_create_(scheme, size_scheme, user_info, size_user_info, host,
                         size_host, port, size_port, path, size_path, query,
                         size_query);
}
#endif
...
static struct ast_uri *uri_parse_and_default(const char *uri,
  ...
  if (res && ast_strlen_zero(ast_uri_port(res))) {
    ...
    struct ast_uri *tmp = ast_uri_copy_replace(
        res, NULL, NULL, NULL, ast_uri_is_secure(res) ? secure_port : port,
        NULL, NULL);
    ao2_ref(res, -1);
    res = tmp;
  }
...
char *ast_uri_make_host_with_port(const struct ast_uri *uri) {
  int host_size = ast_uri_host(uri) ? strlen(ast_uri_host(uri)) : 0;
  /* if there is a port +1 for the colon */
  int port_size = ast_uri_port(uri) ? strlen(ast_uri_port(uri)) + 1 : 0;
  ...
  if (ast_uri_port(uri)) {
    res[host_size] = ':';
    memcpy(res + host_size + 1, ast_uri_port(uri), port_size - 1);
  }
