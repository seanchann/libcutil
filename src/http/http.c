...
/*! Maximum application/json or application/x-www-form-urlencoded body content
 * length. */
...
static AST_RWLIST_HEAD_STATIC(uris,
                              ast_http_uri); /*!< list of supported handlers */
...
static const struct ast_cfhttp_methods_text {
  ...
  const char *text;
    ...
    {AST_HTTP_UNKNOWN, "UNKNOWN"}, {AST_HTTP_GET, "GET"},
    {AST_HTTP_POST, "POST"},       {AST_HTTP_HEAD, "HEAD"},
    {AST_HTTP_PUT, "PUT"},         {AST_HTTP_DELETE, "DELETE"},
    {AST_HTTP_OPTIONS, "OPTIONS"},
};

static char root_dir[128] = {0};

const char *cutil_http_get_static_root_dir(void) { return root_dir; }

void cutil_http_set_static_root_dir(const char *root_dir) {
  snprintf(root_dir, sizeof(root_dir), root_dir);
}
...
static int static_callback(struct ast_tcptls_session_instance *ser,
  ...
  /* Yuck.  I'm not really sold on this, but if you don't deliver static content
     it makes your configuration
     substantially more challenging, but this seems like a rather irritating
     feature creep on Asterisk. */
  ...
  /* Cap maximum length */
  if ((len = strlen(uri) + strlen(cutil_http_get_static_root_dir()) +
             strlen("/static-http/") + 5) > 1024) {
    goto out403;
  }

  path = ast_alloca(len);
  sprintf(path, "%s/static-http/%s", cutil_http_get_static_root_dir(), uri);
  if (stat(path, &st)) {
    goto out404;
  }
  ...
  tv.tv_usec = 0;
  ast_strftime(timebuf, sizeof(timebuf), "%a, %d %b %Y %H:%M:%S GMT",
               ast_localtime(&tv, &tm, "GMT"));
  ...
  /* ast_http_send() frees http_header, so we don't need to do it before
   * returning */
  if (not_modified) {
    ast_http_send(ser, method, 304, "Not Modified", http_header, NULL, 0, 1);
  } else {
    ast_http_send(ser, method, 200, NULL, http_header, NULL, fd,
                  1); /* static content flag is set */
  }
  close(fd);
  return 0;

out404:
  ast_http_error(ser, 404, "Not Found",
                 "The requested URL was not found on this server.");
  return 0;

out403:
  ast_http_request_close_on_completion(ser);
  ast_http_error(ser, 403, "Access Denied",
...
static int httpstatus_callback(struct ast_tcptls_session_instance *ser,
  ...
  ast_str_append(&out, 0,
                 ...
                 "<body bgcolor=\"#ffffff\">\r\n"
                 "<table bgcolor=\"#f1f1f1\" align=\"center\"><tr><td "
                 "bgcolor=\"#e0e0ff\" colspan=\"2\" width=\"500\">\r\n"
  ...
  ast_str_append(&out, 0,
                 ...
                 http_server_name);
  ast_str_append(
      &out, 0, "<tr><td><i>Prefix</i></td><td><b>%s</b></td></tr>\r\n", prefix);
  ast_str_append(&out, 0,
  ...
  if (http_tls_cfg.enabled) {
    ast_str_append(
        &out, 0, "<tr><td><i>SSL Bind Port</i></td><td><b>%s</b></td></tr>\r\n",
...
void ast_http_send(struct ast_tcptls_session_instance *ser,
                   enum ast_http_method method, int status_code,
                   const char *status_title, struct ast_str *http_header,
  ...
  if (!ast_strlen_zero(http_server_name)) {
    ast_str_set(&server_header_field, 0, "Server: %s\r\n", http_server_name);
  }
  ...
  fprintf(ser->f,
          ...
          ast_str_buffer(server_header_field), timebuf,
          close_connection ? "Connection: close\r\n" : "",
          static_content ? "" : "Cache-Control: no-cache, no-store\r\n",
          http_header ? ast_str_buffer(http_header) : "", content_length);
...
void ast_http_create_response(struct ast_tcptls_session_instance *ser,
                              int status_code, const char *status_title,
  ...
  if (!ast_strlen_zero(http_server_name)) {
    ...
    ast_str_set(&server_address, 0, "<address>%s</address>\r\n", server_name);
  }
  ...
  ast_str_set(&out, 0,
              ...
              "%s"
              "</body></html>\r\n",
              status_code, status_title, status_title, text ? text : "",
              ast_str_buffer(server_address));

  ast_free(server_address);

  ast_http_send(ser, AST_HTTP_UNKNOWN, status_code, status_title,
                http_header_data, out, 0, 0);
}

void ast_http_auth(struct ast_tcptls_session_instance *ser, const char *realm,
                   const unsigned long nonce, const unsigned long opaque,
                   int stale, const char *text) {
  int status_code = 401;
  char *status_title = "Unauthorized";
  struct ast_str *http_header_data =
      ast_str_create(DEFAULT_RESPONSE_HEADER_LENGTH);

  if (http_header_data) {
    ast_str_set(&http_header_data, 0,
                "WWW-authenticate: Digest algorithm=MD5, realm=\"%s\", "
                "nonce=\"%08lx\", qop=\"auth\", opaque=\"%08lx\"%s\r\n"
                "Content-type: text/html\r\n",
                realm ? realm : "Asterisk", nonce, opaque,
                stale ? ", stale=true" : "");
  }

  ast_http_create_response(ser, status_code, status_title, http_header_data,
                           text);
}
 ...
 * \return Nothing
 */
static void http_request_tracking_init(
    struct http_worker_private_data *request) {
  ast_set_flags_to(
      &request->flags,
...
static int http_body_discard_contents(struct ast_tcptls_session_instance *ser,
                                      int length, const char *what_getting) {
  int res;
  ...
  while (sizeof(buf) < length) {
    ...
    if (res < 1) {
      ast_log(LOG_WARNING,
              "Short HTTP request %s (Wanted %zu of remaining %d)\n",
 ...
 * \retval -1 on error.
 */
static int http_body_discard_chunk_trailer_headers(
    struct ast_tcptls_session_instance *ser) {
  char header_line[MAX_HTTP_LINE_LENGTH];
...
static char *ast_http_get_contents(int *return_length,
  ...
  for (;;) {
    ...
    if (http_body_read_contents(ser, buf + content_length, chunk_length,
                                "chunk-data") ||
        http_body_check_chunk_sync(ser)) {
      ast_set_flag(&request->flags, HTTP_FLAG_CLOSE_ON_COMPLETION);
      errno = EIO;
...
static int handle_uri(struct ast_tcptls_session_instance *ser, char *uri,
  ...
  AST_RWLIST_TRAVERSE(&uri_redirects, redirect, entry) {
    if (!strcasecmp(uri, redirect->target)) {
      ...
      ast_http_send(ser, method, 302, "Moved Temporarily", http_header, NULL, 0,
                    0);
      break;
    }
  }
...
struct ast_http_auth *ast_http_get_auth(struct ast_variable *headers) {
  ...
  for (v = headers; v; v = v->next) {
    ...
    if (!ast_begins_with(v->value, BASIC_PREFIX)) {
      ast_log(LOG_DEBUG, "Unsupported Authorization scheme\n");
      continue;
    }
 ...
 * be a problem */
    ...
    cnt =
#endif /* AST_DEVMODE */
        ast_base64decode((unsigned char *)decoded, base64, sizeof(decoded) - 1);
...
int ast_http_response_status_line(const char *buf, const char *version,
                                  int code) {
  int status_code;
...
static void *httpd_helper_thread(void *data) {
   ...
   */
  if (setsockopt(ser->fd, IPPROTO_TCP, TCP_NODELAY, (char *)&flags,
                 sizeof(flags)) < 0) {
  ...
  ser->private_data = ao2_alloc_options(sizeof(struct http_worker_private_data),
  ...
  for (;;) {
    ...
    ast_tcptls_stream_set_timeout_inactivity(ser->stream_cookie,
                                             session_inactivity);
...
static int __ast_http_load(int reload) {
  ...
  // cfg = ast_config_load2("http.conf", "http", config_flags);
  // if (!cfg || cfg == CONFIG_STATUS_FILEUNCHANGED || cfg ==
  ...
  /* Apply modern intermediate settings according to the Mozilla OpSec team as
   * of July 30th, 2015 but disable TLSv1 */
  ast_set_flag(&http_tls_cfg.flags,
               AST_SSL_DISABLE_TLSV1 | AST_SSL_SERVER_CIPHER_ORDER);
  ...
  if (!ast_strlen_zero(http_server_instance->config->session_inactivity)) {
    if (ast_parse_arg(http_server_instance->config->session_inactivity,
                      PARSE_INT32 | PARSE_DEFAULT | PARSE_IN_RANGE,
                      &session_inactivity, DEFAULT_SESSION_INACTIVITY, 1,
                      INT_MAX)) {
  ...
  if (!ast_strlen_zero(http_server_instance->config->session_keep_alive)) {
    if (sscanf(http_server_instance->config->session_keep_alive, "%30d",
  ...
  ast_tls_read_conf(&http_tls_cfg, &https_desc, "tlsenable",
                    http_server_instance->config->tlsenable ? "yes" : "no");

  if (http_server_instance->config->tlsenable) {
    ast_tls_read_conf(&http_tls_cfg, &https_desc, "tlsbindaddr",
                      http_server_instance->config->tlsbindaddr);
    ast_tls_read_conf(&http_tls_cfg, &https_desc, "tlscertfile",
                      http_server_instance->config->tlscertfile);
    ast_tls_read_conf(&http_tls_cfg, &https_desc, "tlsprivatekey",
                      http_server_instance->config->tlsprivatekey);
  }

  enabled = http_server_instance->config->enabled;
  if (http_server_instance->config->enabled) {
    if (ast_parse_arg(http_server_instance->config->bindport,
                      PARSE_UINT32 | PARSE_IN_RANGE | PARSE_DEFAULT, &bindport,
    ...
    if (!(num_addrs = ast_sockaddr_resolve(
              &addrs, http_server_instance->config->bindaddr, 0,
              AST_AF_UNSPEC))) {
      ast_log(LOG_WARNING, "Invalid bind address %s\n",
    ...
    if (!ast_strlen_zero(http_server_instance->config->prefix)) {
      newprefix[0] = '/';
      ast_copy_string(newprefix + 1, http_server_instance->config->prefix,
                      sizeof(newprefix) - 1);
    } else {
      newprefix[0] = '\0';
    }
  ...
  if (num_addrs && enabled) {
     ...
     * the non-TLS bindaddress here.
     */
    if (ast_sockaddr_isnull(&https_desc.local_address) &&
        http_desc.accept_fd != -1) {
      ast_sockaddr_copy(&https_desc.local_address, &http_desc.local_address);
      /* Of course, we can't use the same port though.
       * Since no bind address was specified, we just use the
       * default TLS port
  ...
  if (http_tls_was_enabled && !http_tls_cfg.enabled) {
    ast_tcptls_server_stop(&https_desc);
  } else if (http_tls_cfg.enabled &&
             !ast_sockaddr_isnull(&https_desc.local_address)) {
    /* We can get here either because a TLS-specific address was specified
     * or because we copied the non-TLS address here. In the case where
     * we read an explicit address from the config, there may have been
     * no port specified, so we'll just use the default TLS port.
...
static char *handle_show_http(struct ast_cli_entry *e, int cmd,
  ...
  if (AST_RWLIST_EMPTY(&uris)) {
    ...
    ast_cli(a->fd, "%s/%s%s => %s\n", prefix, urih->uri,
            (urih->has_subtree ? "/..." : ""), urih->description);
  }
  AST_RWLIST_UNLOCK(&uris);
...
int ast_http_reload(void) { return __ast_http_load(1); }
...
static int ast_http_config_init(struct http_server *self,
  ...
  if (config->tlsenable) {
    ...
    snprintf(cfg->tlsprivatekey, sizeof(cfg->tlsprivatekey),
