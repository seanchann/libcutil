...
//#include "libcutil/file.h"
#include "libcutil/unaligned.h"
#include "libcutil/uri.h"
...
/*! \brief Default reconstruction size for multi-frame payload reconstruction.
 * If exceeded the next frame will start a
 *         payload.
 */
...
struct ast_websocket {
  ...
  enum ast_websocket_opcode
      opcode;         /*!< Cached opcode for multi-frame messages */
  size_t payload_len; /*!< Length of the payload */
  char *payload;      /*!< Pointer to the payload */
  size_t reconstruct; /*!< Number of bytes before a reconstructed payload will
                         be returned and a new one started */
  int timeout;        /*!< The timeout for operations on the socket */
  unsigned int secure : 1;  /*!< Bit to indicate that the transport is secure */
  unsigned int closing : 1; /*!< Bit to indicate that the session is in the
                               process of being closed */
  unsigned int
      close_sent : 1; /*!< Bit to indicate that the session close opcode has
                         been sent and no further data will be sent */
  struct websocket_client
      *client; /*!< Client object when connected as a client websocket */
};
...
static int protocol_cmp_fn(void *obj, void *arg, int flags) {
  ...
  return !strcasecmp(protocol1->name,
                     flags & OBJ_KEY ? protocol : protocol2->name)
             ? CMP_MATCH | CMP_STOP
             : 0;
}
...
static struct ast_websocket_server *websocket_server_create_impl(
    void (*dtor)(void *)) {
  ...
  server->protocols = ao2_container_alloc(MAX_PROTOCOL_BUCKETS,
                                          protocol_hash_fn, protocol_cmp_fn);
  if (!server->protocols) {
    return NULL;
  }
...
struct ast_websocket_server *AST_OPTIONAL_API_NAME(ast_websocket_server_create)(
    void) {
  // seanchann remove
...
struct ast_websocket_protocol *AST_OPTIONAL_API_NAME(
    ast_websocket_sub_protocol_alloc)(const char *name) {
...
int AST_OPTIONAL_API_NAME(ast_websocket_server_add_protocol)(
    struct ast_websocket_server *server, const char *name,
...
int AST_OPTIONAL_API_NAME(ast_websocket_server_add_protocol2)(
...
int AST_OPTIONAL_API_NAME(ast_websocket_close)(struct ast_websocket *session,
  ...
  char frame[4] = {
      0,
  }; /* The header is 2 bytes and the reason code takes up another 2 bytes */
  int res;
  ...
  /* If an error occurred when trying to close this connection explicitly
   * terminate it now.
  ...
  if (res) {
    ...
    session->f = NULL;
    ast_verb(2,
             "WebSocket connection %s '%s' forcefully closed due to fatal "
             "write error\n",
             session->client ? "to" : "from",
             ast_sockaddr_stringify(&session->address));
  }
...
int AST_OPTIONAL_API_NAME(ast_websocket_write)(struct ast_websocket *session,
  ...
  if (ast_careful_fwrite(session->f, session->fd, frame, frame_size,
                         session->timeout)) {
    ao2_unlock(session);
    /* 1011 - server terminating connection due to not being able to fulfill the
     * request */
    ast_debug(
        1, "Closing WS with 1011 because we can't fulfill a write request\n");
...
void AST_OPTIONAL_API_NAME(ast_websocket_reconstruct_enable)(
    struct ast_websocket *session, size_t bytes) {
  session->reconstruct = MIN(bytes, MAXIMUM_RECONSTRUCTION_CEILING);
}

void AST_OPTIONAL_API_NAME(ast_websocket_reconstruct_disable)(
...
struct ast_sockaddr *AST_OPTIONAL_API_NAME(ast_websocket_remote_address)(
    struct ast_websocket *session) {
  return &session->address;
}

int AST_OPTIONAL_API_NAME(ast_websocket_is_secure)(
    struct ast_websocket *session) {
  return session->secure;
}

int AST_OPTIONAL_API_NAME(ast_websocket_set_nonblock)(
    struct ast_websocket *session) {
  int flags;
...
int AST_OPTIONAL_API_NAME(ast_websocket_set_timeout)(
    struct ast_websocket *session, int timeout) {
 ...
 *
 * We have to keep in mind during this function that the fact that session->fd
 * seems ready
 * (via poll) does not necessarily mean we have application data ready, because
 * in the case
 * of an SSL socket, there is some encryption data overhead that needs to be
 * read from the
 * TCP socket, so poll() may say there are bytes to be read, but whether it is
 * just 1 byte
 * or N bytes we do not know that, and we do not know how many of those bytes
 * (if any) are
 * for application data (for us) and not just for the SSL protocol consumption
 *
 * There used to be a couple of nasty bugs here that were fixed in last
 * refactoring but I
 * want to document them so the constraints are clear and we do not re-introduce
 * them:
 *
 * - This function would incorrectly assume that fread() would necessarily
 * return more than
 *   1 byte of data, just because a websocket frame is always >= 2 bytes, but
 * the thing
 *   is we're dealing with a TCP bitstream here, we could read just one byte and
 * that's normal.
 *   The problem before was that if just one byte was read, the function bailed
 * out and returned
 *   an error, effectively dropping the first byte of a websocket frame header!
 *
 * - Another subtle bug was that it would just read up to MAX_WS_HDR_SZ (14
 * bytes) via fread()
 *   then assume that executing poll() would tell you if there is more to read,
 * but since
 *   we're dealing with a buffered stream (session->f is a FILE*), poll would
 * say there is
 *   nothing else to read (in the real tcp socket session->fd) and we would get
 * stuck here
 *   without processing the rest of the data in session->f internal buffers
 * until another packet
 *   came on the network to unblock us!
 *
 * Note during the header parsing stage we try to read in small chunks just what
 * we need, this
 * is buffered data anyways, no expensive syscall required most of the time ...
 */
static inline int ws_safe_read(struct ast_websocket *session, char *buf,
                               int len, enum ast_websocket_opcode *opcode) {
  size_t rlen;
  int xlen = len;
  char *rbuf = buf;
  int sanity = 10;
...
int AST_OPTIONAL_API_NAME(ast_websocket_read)(struct ast_websocket *session,
  ...
  *fragmented = 0;
  ...
  /* ok, now we have the first 2 bytes, so we know some flags, opcode and
   * payload length (or whether payload length extension will be required) */
  *opcode = buf[0] & 0xf;
  *payload_len = buf[1] & 0x7f;
  if (*opcode == AST_WEBSOCKET_OPCODE_TEXT ||
      *opcode == AST_WEBSOCKET_OPCODE_BINARY ||
      *opcode == AST_WEBSOCKET_OPCODE_CONTINUATION ||
      *opcode == AST_WEBSOCKET_OPCODE_PING ||
      *opcode == AST_WEBSOCKET_OPCODE_PONG) {
    fin = (buf[0] >> 7) & 1;
    mask_present = (buf[1] >> 7) & 1;

    /* Based on the mask flag and payload length, determine how much more we
     * need to read before start parsing the rest of the header */
    ...
    *payload = &buf[frame_size]; /* payload will start here, at the end of the
                                    options, if any */
    frame_size =
        frame_size + (*payload_len); /* final frame size is header + optional
    ...
    /* Per the RFC for PING we need to send back an opcode with the application
     * data as received */
    if ((*opcode == AST_WEBSOCKET_OPCODE_PING) &&
        (ast_websocket_write(session, AST_WEBSOCKET_OPCODE_PONG, *payload,
                             *payload_len))) {
      *payload_len = 0;
      ast_websocket_close(session, 1009);
      return 0;
    ...
    if (!fin && session->reconstruct &&
        (session->payload_len < session->reconstruct)) {
      /* If this is not a final message we need to defer returning it until
       * later */
      if (*opcode != AST_WEBSOCKET_OPCODE_CONTINUATION) {
      ...
      *payload = NULL;
      ...
      if (*opcode == AST_WEBSOCKET_OPCODE_CONTINUATION) {
        if (!fin) {
          /* If this was not actually the final message tell the user it is
           * fragmented so they can deal with it accordingly */
          *fragmented = 1;
        } else {
    ...
    /* Make the payload available so the user can look at the reason code if
     * they so desire */
    if ((*payload_len) &&
        (new_payload = ast_realloc(session->payload, *payload_len))) {
    ...
    /* We received an opcode that we don't understand, the RFC states that 1003
     * is for a type of data that can't be accepted... opcodes
     * fit that, I think. */
...
static void websocket_bad_request(struct ast_tcptls_session_instance *ser) {
  ...
  ast_http_send(ser, AST_HTTP_UNKNOWN, 400, "Bad Request", http_header, NULL, 0,
                0);
}

int AST_OPTIONAL_API_NAME(ast_websocket_uri_cb)(
    struct ast_tcptls_session_instance *ser, const struct ast_http_uri *urih,
    const char *uri, enum ast_http_method method, struct ast_variable *get_vars,
    struct ast_variable *headers) {
  struct ast_variable *v;
  char *upgrade = NULL, *key = NULL, *key1 = NULL, *key2 = NULL, *protos = NULL,
  ...
  if (method != AST_HTTP_GET) {
    ast_http_error(ser, 501, "Not Implemented",
  ...
  if (!upgrade || strcasecmp(upgrade, "websocket")) {
    ast_log(LOG_WARNING,
            "WebSocket connection from '%s' could not be accepted - did not "
            "request WebSocket\n",
    ...
    if (!protocol_handler) {
      ...
      ast_log(LOG_WARNING,
              "WebSocket connection from '%s' could not be accepted - no "
              "protocols requested\n",
              ast_sockaddr_stringify(&ser->remote_address));
     ...
     * http://tools.ietf.org/html/draft-hixie-thewebsocketprotocol-76 and
     * http://tools.ietf.org/html/draft-ietf-hybi-thewebsocketprotocol-00 -- not
     * currently supported*/
    ast_log(LOG_WARNING,
            "WebSocket connection from '%s' could not be accepted - "
            "unsupported version '00/76' chosen\n",
            ast_sockaddr_stringify(&ser->remote_address));
    websocket_bad_request(ser);
    return 0;
  }

  /* Iterate through the requested protocols trying to find one that we have a
   * handler for */
  while (!protocol_handler && (protocol = strsep(&requested_protocols, ","))) {
    protocol_handler =
        ao2_find(server->protocols, ast_strip(protocol), OBJ_KEY);
  }
  ...
  if (version == 7 || version == 8 || version == 13) {
    ...
    if (!key ||
        strlen(key) + strlen(WEBSOCKET_GUID) + 1 >
            8192) { /* no stack overflows please */
      websocket_bad_request(ser);
    ...
    if (protocol) {
      fprintf(ser->f,
              ...
              upgrade, websocket_combine_key(key, base64, sizeof(base64)),
              protocol);
    } else {
      fprintf(ser->f,
              "HTTP/1.1 101 Switching Protocols\r\n"
    ...
    /* Specification defined in
     * http://tools.ietf.org/html/draft-hixie-thewebsocketprotocol-75 or
     * completely unknown */
    ast_log(LOG_WARNING,
            "WebSocket connection from '%s' could not be accepted - "
            "unsupported version '%d' chosen\n",
            ast_sockaddr_stringify(&ser->remote_address),
            version ? version : 75);
    websocket_bad_request(ser);
    ao2_ref(protocol_handler, -1);
  ...
  if (setsockopt(ser->fd, SOL_SOCKET, SO_KEEPALIVE, &flags, sizeof(flags))) {
    ast_log(LOG_WARNING,
            "WebSocket connection from '%s' could not be accepted - failed to "
            "enable keepalive\n",
            ast_sockaddr_stringify(&ser->remote_address));
    websocket_bad_request(ser);
    ao2_ref(session, -1);
  ...
  ast_verb(2,
           "WebSocket connection from '%s' for protocol '%s' accepted using "
           "version '%d'\n",
...
/*! \brief Simple echo implementation which echoes received text and binary
 * frames */
static void websocket_echo_callback(struct ast_websocket *session,
  ...
  while ((res = ast_wait_for_input(ast_websocket_fd(session), -1)) > 0) {
    ...
    if (ast_websocket_read(session, &payload, &payload_len, &opcode,
                           &fragmented)) {
      /* We err on the side of caution and terminate the session if any error
       * occurs */
...
int AST_OPTIONAL_API_NAME(ast_websocket_add_protocol)(
    const char *name, ast_websocket_callback callback) {
...
int AST_OPTIONAL_API_NAME(ast_websocket_remove_protocol)(
    const char *name, ast_websocket_callback callback) {
...
static struct ast_tcptls_session_args *websocket_client_args_create(
  ...
  struct ast_tcptls_session_args *args =
      ao2_alloc(sizeof(*args), websocket_client_args_destroy);
...
static struct ast_websocket *websocket_client_create(
  ...
  if (!(ws->client =
            ao2_alloc(sizeof(*ws->client), websocket_client_destroy))) {
  ...
  if (websocket_client_parse_uri(uri, &ws->client->host,
                                 &ws->client->resource_name)) {
    ao2_ref(ws, -1);
  ...
  if (!(ws->client->args =
            websocket_client_args_create(ws->client->host, tls_cfg, result))) {
    ao2_ref(ws, -1);
    return NULL;
  }
...
const char *AST_OPTIONAL_API_NAME(ast_websocket_client_accept_protocol)(
...
static enum ast_websocket_result websocket_client_handshake_get_response(
  ...
  if ((res = websocket_client_handle_response_code(
           client, ast_http_response_status_line(buf, "HTTP/1.1", 101))) !=
      WS_OK) {
    return res;
  }
  ...
  while (fgets(buf, sizeof(buf), client->ser->f)) {
    ...
    if (!has_upgrade &&
        (has_upgrade =
             ast_http_header_match(name, "upgrade", value, "websocket")) < 0) {
      return WS_HEADER_MISMATCH;
    } else if (!has_connection &&
               (has_connection = ast_http_header_match(name, "connection",
                                                       value, "upgrade")) < 0) {
      return WS_HEADER_MISMATCH;
    } else if (!has_accept &&
               (has_accept = ast_http_header_match(
                    name, "sec-websocket-accept", value,
                    websocket_combine_key(client->key, base64,
                                          sizeof(base64)))) < 0) {
      return WS_HEADER_MISMATCH;
    } else if (!has_protocol && (has_protocol = ast_http_header_match_in(
  ...
  return has_upgrade && has_connection && has_accept ? WS_OK
                                                     : WS_HEADER_MISSING;
}
...
static enum ast_websocket_result websocket_client_handshake(
  ...
  if (fprintf(
          ...
          client->version, client->host, client->key, protocols) < 0) {
...
struct ast_websocket *AST_OPTIONAL_API_NAME(ast_websocket_client_create)(
    ...
    enum ast_websocket_result *result) {
  struct ast_websocket *ws =
      websocket_client_create(uri, protocols, tls_cfg, result);
...
int AST_OPTIONAL_API_NAME(ast_websocket_write_string)(struct ast_websocket *ws,
   ...
   */
  return ast_websocket_write(ws, AST_WEBSOCKET_OPCODE_TEXT, (char *)buf, len);
}
...
// AST_MODFLAG_LOAD_ORDER, "HTTP WebSocket Support",
