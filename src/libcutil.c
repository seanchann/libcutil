  ...
  if (!g_libcutil) {
    ...
    if (g_libcutil) {
      ...
      if (g_libcutil->logger_cfg) {
        ...
        snprintf(console_line->chan_name, sizeof(console_line->chan_name),
                 "console");
        snprintf(console_line->chan_value, sizeof(console_line->chan_value),
                 "notice,warning,error");

        AST_LIST_HEAD_INIT_NOLOCK(&g_libcutil->logger_cfg->lines);
        AST_LIST_INSERT_TAIL(&g_libcutil->logger_cfg->lines, console_line,
                             list);
...
err:
  fprintf(stderr, "Unable to calloc any more memory for libcutil\n");
  exit(1);
}
...
static struct libcutil *libcutil_instance(void) { return g_libcutil; }
...
void libcutil_set_config_dir(const char *dir) {
  ...
  snprintf(instance->config_CONFIG_DIR, sizeof(instance->config_CONFIG_DIR),
           "%s", dir);
}
...
void libcutil_set_config_log_dir(const char *dir) {
  ...
  snprintf(instance->config_LOG_DIR, sizeof(instance->config_LOG_DIR), "%s",
           dir);
}
...
void libcutil_set_config_system_name(const char *system_name) {
  ...
  snprintf(instance->config_SYSTEM_NAME, sizeof(instance->config_SYSTEM_NAME),
           "%s", system_name);
}
...
int libcutil_set_config_socket(void) {
  ...
  if (!ast_strlen_zero(libcutil_get_config_run_dir())) {
    snprintf(instance->config_SOCKET, sizeof(instance->config_SOCKET),
...
void libcutil_set_config_run_user(const char *run_user) {
  ...
  snprintf(instance->config_RUN_USER, sizeof(instance->config_RUN_USER), "%s",
           run_user);
}
...
void libcutil_set_config_run_group(const char *run_group) {
  ...
  snprintf(instance->config_RUN_GROUP, sizeof(instance->config_RUN_GROUP), "%s",
           run_group);
}
...
void libcutil_set_config_run_dir(const char *dir) {
  ...
  snprintf(instance->config_RUN_DIR, sizeof(instance->config_RUN_DIR), "%s",
           dir);
}
...
void libcutil_logger_append_logfiles_line(char *name, char *value) {
  ...
  snprintf(console_line->chan_name, sizeof(console_line->chan_name), "%s",
           name);

  snprintf(console_line->chan_value, sizeof(console_line->chan_value), "%s",
           value);
