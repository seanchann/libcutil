...
static char *ari_show(struct ast_cli_entry *e, int cmd,
...
static int show_users_cb(void *obj, void *arg, int flags) {
  ...
  ast_cli(a->fd, "%-4s  %s\n", AST_CLI_YESNO(user->read_only), user->username);
  return 0;
}
...
static int complete_ari_user_search(void *obj, void *arg, void *data,
                                    int flags) {
...
static char *ari_show_user(struct ast_cli_entry *e, int cmd,
                           struct ast_cli_args *a) {
  RAII_VAR(struct ast_ari_conf *, conf, NULL, ao2_cleanup);
  RAII_VAR(struct ast_ari_conf_user *, user, NULL, ao2_cleanup);
...
static char *ari_mkpasswd(struct ast_cli_entry *e, int cmd,
                          struct ast_cli_args *a) {
  ...
  ast_cli(a->fd, "; Copy the following two lines into ari.conf\n");
