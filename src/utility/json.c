...
int ast_json_dump_str_format(struct ast_json *root, struct ast_str **dst,
   ...
   * See http://www.digip.org/jansson/doc/2.4/portability.html#thread-safety. */
  SCOPED_JSON_LOCK(root);
  return json_dump_callback((json_t *)root, write_to_ast_str, dst,
                            dump_flags(format));
}
...
static struct ast_json* json_party_number(struct ast_party_number *number)
{
  ...
  return ast_json_pack("{s: s, s: i, s: i, s: s}",
                       ...
                       "presentation_txt",
                       ast_describe_caller_presentation(number->presentation));
}
...
static struct ast_json* json_party_name(struct ast_party_name *name)
{
  ...
  return ast_json_pack("{s: s, s: s, s: i, s: s}",
                       ...
                       "character_set",
                       ast_party_name_charset_describe(name->char_set),
                       "presentation", name->presentation,
                       "presentation_txt",
                       ast_describe_caller_presentation(name->presentation));
}

static struct ast_json* json_party_subaddress(
...
struct ast_json* ast_json_party_id(struct ast_party_id *party)
{
  ...
  if (party->number.valid
      && ast_json_object_set(json_party_id, "number",
                             json_party_number(&party->number))) {
    return NULL;
  }

  /* Party name */
  if (party->name.valid
      && ast_json_object_set(json_party_id, "name",
                             json_party_name(&party->name))) {
    return NULL;
  }

  /* Party subaddress */
  if (party->subaddress.valid
      && ast_json_object_set(json_party_id, "subaddress",
                             json_party_subaddress(&party->subaddress))) {
    return NULL;
  }
