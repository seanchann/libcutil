...
void ast_tcptls_stream_set_timeout_sequence(struct ast_tcptls_stream *stream,
                                            struct timeval start, int timeout) {
  ast_assert(stream != NULL);
 ...
 * \retval NULL on error.
 */
static FILE *tcptls_stream_fopen(struct ast_tcptls_stream *stream, SSL *ssl,
                                 int fd, int timeout) {
  FILE *fp;

#if defined(HAVE_FOPENCOOKIE) /* the glibc/linux interface */
  static const cookie_io_functions_t cookie_funcs = {
      tcptls_stream_read, tcptls_stream_write, NULL, tcptls_stream_close};
...
HOOK_T ast_tcptls_server_read(
    struct ast_tcptls_session_instance *tcptls_session, void *buf,
    size_t count) {
  if (!tcptls_session->stream_cookie ||
      tcptls_session->stream_cookie->fd == -1) {
...
HOOK_T ast_tcptls_server_write(
    struct ast_tcptls_session_instance *tcptls_session, const void *buf,
    size_t count) {
  if (!tcptls_session->stream_cookie ||
      tcptls_session->stream_cookie->fd == -1) {
...
static int check_tcptls_cert_name(ASN1_STRING *cert_str, const char *hostname,
  ...
  if (strlen((char *)str) != ret) {
    ast_log(LOG_WARNING,
            "Invalid certificate %s length (contains NULL bytes?)\n", desc);
 ...
 */
  ...
  */
  if (!tcptls_session->parent->tls_cfg) {
    ...
    } else if ((tcptls_session->f = tcptls_stream_fopen(
                    tcptls_session->stream_cookie, tcptls_session->ssl,
                    tcptls_session->fd, -1))) {
      if ((tcptls_session->client &&
           !ast_test_flag(&tcptls_session->parent->tls_cfg->flags,
                          AST_SSL_DONT_VERIFY_SERVER)) ||
          (!tcptls_session->client &&
           ast_test_flag(&tcptls_session->parent->tls_cfg->flags,
        ...
        if (!ast_test_flag(&tcptls_session->parent->tls_cfg->flags,
          ...
          for (;;) {
            ...
            /* XXX Probably should do a gethostbyname on the hostname and
            ...
            if (!check_tcptls_cert_name(str, tcptls_session->parent->hostname,
                                        "common name")) {
              found = 1;
              break;
            }
            ...
            if (alt_names != NULL) {
              ...
              for (pos = 0; pos < alt_names_count; pos++) {
                const GENERAL_NAME *alt_name =
                    sk_GENERAL_NAME_value(alt_names, pos);

                if (alt_name->type != GEN_DNS) {
                  continue;
                }

                if (!check_tcptls_cert_name(alt_name->d.dNSName,
  ...
  if (!tcptls_session->f) {
    ...
    if (tcptls_session->parent->tls_cfg) {
      ast_log(LOG_ERROR,
              "Attempted a TLS connection without OpenSSL support. This will "
              "not work!\n");
    }
#endif
    ao2_ref(tcptls_session, -1);
...
void *ast_tcptls_server_root(void *data) {
  ...
  for (;;) {
    ...
    ast_log(LOG_NOTICE, "New tcp connect, accept fd %d\r\n", fd);
    tcptls_session =
    ...
    /* This thread is now the only place that controls the single ref to
     * tcptls_session */
    if (ast_pthread_create_detached_background(
            &launched, NULL, handle_tcptls_connection, tcptls_session)) {
...
static int __ssl_setup(struct ast_tls_config *cfg, int client) {
  ...
  if (client) {
    ...
    if (ast_test_flag(&cfg->flags, AST_SSL_SSLV2_CLIENT)) {
      ast_log(LOG_WARNING,
              "Usage of SSLv2 is discouraged due to known vulnerabilities. "
              "Please use 'tlsv1' or leave the TLS method unspecified!\n");
      ...
      ast_log(LOG_WARNING,
              "Usage of SSLv3 is discouraged due to known vulnerabilities. "
              "Please use 'tlsv1' or leave the TLS method unspecified!\n");
  ...
  ast_log(
      LOG_WARNING,
      "Your version of OpenSSL leaves you potentially vulnerable "
  ...
  SSL_CTX_set_verify(cfg->ssl_ctx,
                     ast_test_flag(&cfg->flags, AST_SSL_VERIFY_CLIENT)
  ...
  if (!ast_strlen_zero(cfg->certfile)) {
    ...
    if ((SSL_CTX_use_PrivateKey_file(cfg->ssl_ctx, tmpprivate,
                                     SSL_FILETYPE_PEM) == 0) ||
  ...
  if (!ast_strlen_zero(cfg->cafile) || !ast_strlen_zero(cfg->capath)) {
    if (SSL_CTX_load_verify_locations(cfg->ssl_ctx, S_OR(cfg->cafile, NULL),
                                      S_OR(cfg->capath, NULL)) == 0) {
...
#define SSL_CTRL_SET_ECDH_AUTO 94
#endif
  /* SSL_CTX_set_ecdh_auto(cfg->ssl_ctx, on); requires OpenSSL 1.0.2 which
   * wraps: */
  if (SSL_CTX_ctrl(cfg->ssl_ctx, SSL_CTRL_SET_ECDH_AUTO, 1, NULL)) {
    ...
    if (ecdh != NULL) {
      if (SSL_CTX_set_tmp_ecdh(cfg->ssl_ctx, ecdh)) {
        ast_verb(2,
                 "TLS/SSL ECDH initialized (secp256r1), faster PFS "
  ...
  ast_verb(2, "TLS/SSL certificate ok\n"); /* We should log which one that is
                                              ok. This message doesn't really
                                              make sense in production use */
  return 1;
#endif
}
...
struct ast_tcptls_session_instance *ast_tcptls_client_create(
  ...
  desc->accept_fd =
      socket(ast_sockaddr_is_ipv6(&desc->remote_address) ? AF_INET6 : AF_INET,
  ...
  if (!ast_sockaddr_isnull(&desc->local_address)) {
    ...
    if (ast_bind(desc->accept_fd, &desc->local_address)) {
      ast_log(LOG_ERROR, "Unable to bind %s to %s: %s\n", desc->name,
              ast_sockaddr_stringify(&desc->local_address), strerror(errno));
      goto error;
    }
  }
  ...
  ast_sockaddr_copy(&tcptls_session->remote_address, &desc->remote_address);
...
void ast_tcptls_server_start(struct ast_tcptls_session_args *desc) {
  ...
  if (desc->tls_cfg) {
    ...
    if (stat(desc->tls_cfg->certfile, &st) ||
        NULL == (str = ast_read_textfile(desc->tls_cfg->certfile))) {
      memset(hash, 0, 41);
    } else {
    ...
    if (stat(desc->tls_cfg->pvtfile, &st) ||
        NULL == (str = ast_read_textfile(desc->tls_cfg->pvtfile))) {
      memset(hash, 0, 41);
    } else {
      ast_sha1_hash(hash, str);
    }
    ...
    if (stat(desc->tls_cfg->cafile, &st) ||
        NULL == (str = ast_read_textfile(desc->tls_cfg->cafile))) {
      memset(hash, 0, 41);
    } else {
      ast_sha1_hash(hash, str);
    }
    ...
    if (!desc->old_tls_cfg) { /* No previous configuration */
    ...
    } else if (memcmp(desc->tls_cfg->certhash, desc->old_tls_cfg->certhash,
                      41)) {
      tls_changed = 1;
      ...
      tls_changed = 1;
    } else if (memcmp(&desc->tls_cfg->flags, &desc->old_tls_cfg->flags,
                      sizeof(desc->tls_cfg->flags))) {
      tls_changed = 1;
    }
  ...
  /* Do nothing if nothing has changed */
  if (!tls_changed &&
      !ast_sockaddr_cmp(&desc->old_address, &desc->local_address)) {
    ast_debug(1, "Nothing changed in %s\n", desc->name);
    return;
  }
  ...
  desc->accept_fd =
      socket(ast_sockaddr_is_ipv6(&desc->local_address) ? AF_INET6 : AF_INET,
             SOCK_STREAM, 0);
  if (desc->accept_fd < 0) {
    ast_log(LOG_ERROR, "Unable to allocate socket for %s: %s\n", desc->name,
            strerror(errno));
  ...
  flags = fcntl(desc->accept_fd, F_GETFL);
  fcntl(desc->accept_fd, F_SETFL, flags | O_NONBLOCK);
  if (ast_pthread_create_background(&desc->master, NULL, desc->accept_fn,
                                    desc)) {
  ...
  if (desc->old_tls_cfg) {
    ...
    memcpy(&desc->old_tls_cfg->flags, &desc->tls_cfg->flags,
           sizeof(desc->old_tls_cfg->flags));
  }
...
void ast_tcptls_close_session_file(
  ...
  if (tcptls_session->f) {
    ...
    ast_log(LOG_ERROR,
            "ast_tcptls_close_session_file invoked on session instance without "
...
int ast_tls_read_conf(struct ast_tls_config *tls_cfg,
  ...
  if (!strcasecmp(varname, "tlsenable") || !strcasecmp(varname, "sslenable")) {
  ...
  } else if (!strcasecmp(varname, "tlscertfile") ||
             !strcasecmp(varname, "sslcert") ||
             !strcasecmp(varname, "tlscert")) {
    ast_free(tls_cfg->certfile);
    tls_cfg->certfile = ast_strdup(value);
  } else if (!strcasecmp(varname, "tlsprivatekey") ||
             !strcasecmp(varname, "sslprivatekey")) {
    ast_free(tls_cfg->pvtfile);
    tls_cfg->pvtfile = ast_strdup(value);
  } else if (!strcasecmp(varname, "tlscipher") ||
             !strcasecmp(varname, "sslcipher")) {
  ...
  } else if (!strcasecmp(varname, "tlsbindaddr") ||
             !strcasecmp(varname, "sslbindaddr")) {
    if (ast_parse_arg(value, PARSE_ADDR, &tls_desc->local_address))
      ast_log(LOG_ERROR, "Invalid %s '%s'\n", varname, value);
  } else if (!strcasecmp(varname, "tlsclientmethod") ||
             !strcasecmp(varname, "sslclientmethod")) {
    ...
    ast_set2_flag(&tls_cfg->flags, ast_true(value),
