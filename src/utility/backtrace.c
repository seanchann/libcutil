...
#  include <dlfcn.h>
#  include <bfd.h>
# endif /* if defined(HAVE_DLADDR) && defined(HAVE_BFD) &&
...
# if defined(BETTER_BACKTRACES)
  ...
  size_t       strings_size;
  size_t      *eachlen;
# endif /* if defined(BETTER_BACKTRACES) */
...
# if defined(BETTER_BACKTRACES)
  ...
  for (stackfr = 0; stackfr < num_frames; stackfr++) {
    ...
    if (strcmp(dli.dli_fname, "asterisk") == 0) {
      ...
      if (!(dli.dli_fname =
              ast_utils_which("asterisk", asteriskpath, sizeof(asteriskpath)))) {
    ...
    if ((bfdobj = bfd_openr(dli.dli_fname, NULL)) &&
      ...
      for (section = bfdobj->sections; section; section = section->next) {
        ...
        if (!bfd_find_nearest_line(bfdobj, section, syms, offset - section->vma,
                                   &file, &func, &line)) {
          continue;
        }

        /* file can possibly be null even with a success result from
        ...
        if ((lastslash = strrchr(file, '/'))) {
          ...
          for (prevslash = lastslash - 1; *prevslash != '/' && prevslash >= file;
               prevslash--) {}
