 ...
 *
 * The original source from this file may be obtained from
...
# ifdef HAVE_O_SYMLINK
  int fds;
# else  /* ifdef HAVE_O_SYMLINK */
  DIR *dir;
# endif /* defined(HAVE_O_SYMLINK) */
#else  /* ifdef HAVE_INOTIFY */
  time_t mtime[2];
#endif /* ifdef HAVE_INOTIFY */
...
static const char *     getqzname        P((const char *strp,
                                            const int delim));
static const char *     getnum           P((const char *strp, int *nump,
                                            int min,
                                            int max));
...
static struct ast_tm *  gmtsub   P((const struct timeval *timep,
                                    long offset,
                                    struct ast_tm        *tmp));
static struct ast_tm *  localsub P((const struct timeval *timep,
                                    long offset,
                                    struct ast_tm        *tmp,
...
static void* inotify_daemon(void *data)
{
  ...
  for (; /*ever*/;) {
    ...
    if (((res =
            read(inotify_fd, iev,
                 real_sizeof_iev)) < sizeof(*iev)) && (res > 0)) {
...
static void add_notify(struct state *sp, const char *path)
{
  ...
  if (inotify_thread == AST_PTHREADT_NULL) {
    ...
    if (!(ast_pthread_create_background(&inotify_thread, NULL, inotify_daemon,
                                        NULL))) {
  ...
  if (inotify_fd > -1) {
    ...
    if (readlink(path, fullpath, sizeof(fullpath) - 1) != -1) {
      ...
      sp->wd[1] = inotify_add_watch(inotify_fd,
                                    fullpath,
                                    IN_ATTRIB | IN_DELETE_SELF | IN_MODIFY | IN_MOVE_SELF |
                                    IN_CLOSE_WRITE);
    } else {
      sp->wd[1] = -1;
    }

    /* or if the symlink itself changes (or the real file is here, if path is
       not a symlink) */
    sp->wd[0] = inotify_add_watch(inotify_fd,
                                  path,
                                  IN_ATTRIB | IN_DELETE_SELF | IN_MODIFY | IN_MOVE_SELF | IN_CLOSE_WRITE
# ifdef IN_DONT_FOLLOW /* Only defined in glibc 2.5 and above */
                                  | IN_DONT_FOLLOW
# endif /* ifdef IN_DONT_FOLLOW */
                                  );
  }
}
...
# else  /* ifdef NOTE_TRUNCATE */
#  define EVVN_NOTES_BITS                                             \
  (NOTE_DELETE | NOTE_WRITE | NOTE_EXTEND | NOTE_REVOKE | NOTE_ATTRIB \
   | NOTE_RENAME | NOTE_LINK)
# endif /* ifdef NOTE_TRUNCATE */
...
static void add_notify(struct state *sp, const char *path)
{
  ...
  if (inotify_thread == AST_PTHREADT_NULL) {
    ...
    if (!(ast_pthread_create_background(&inotify_thread, NULL, kqueue_daemon,
                                        NULL))) {
      /* Give the thread a chance to initialize */
      ast_cond_wait(&initialization, &initialization_lock);
    }
...
# ifdef HAVE_O_SYMLINK
  ...
  if ((readlink(path, watchdir,
                sizeof(watchdir) - 1) != -1) &&
      ((sp->fds = open(path, O_RDONLY | O_SYMLINK
#  ifdef HAVE_O_EVTONLY
                       |
                       O_EVTONLY
#  endif /* ifdef HAVE_O_EVTONLY */
                       ))
       >= 0)) {
    EV_SET(&kev,
           sp->fds,
...
# else  /* ifdef HAVE_O_SYMLINK */
  ...
  if (readlink(path, watchdir, sizeof(watchdir) - 1) != -1) {
    /* Special -- watch the directory for changes, because we cannot directly
       watch a symlink */
    char *slash;
...
static void add_notify(struct state *sp, const char *path)
{
  ...
  if (inotify_thread == AST_PTHREADT_NULL) {
    ...
    if (!(ast_pthread_create_background(&inotify_thread, NULL, notify_daemon,
                                        NULL))) {
      /* Give the thread a chance to initialize */
      ast_cond_wait(&initialization, &initialization_lock);
    }
...
void ast_localtime_wakeup_monitor(struct ast_test *info)
{
  ...
  { .tv_sec = wait_now.tv_sec + 2, .tv_nsec = wait_now.tv_usec * 1000 };
...
static time_t detzcode64(const char *const codep)
{
  ...
  for (i = 0; i < 8; ++i) result = result * 256 + (codep[i] & 0xff);
  return result;
}
...
static int tzload(const char *name, struct state *const sp, const int doextend)
{
  ...
  if ((close(fid) < 0) || (nread < sizeof u.tzhead)) return -1;
  ...
  for (stored = 4; stored <= 8; stored *= 2) {
    ...
    if ((sp->leapcnt < 0) || (sp->leapcnt > TZ_MAX_LEAPS) ||
        ...
        ((ttisgmtcnt != sp->typecnt) && (ttisgmtcnt != 0))) return -1;
    ...
    for (i = 0; i < sp->typecnt; ++i) {
      ...
      if ((ttisp->tt_abbrind < 0) ||
          (ttisp->tt_abbrind > sp->charcnt)) return -1;
    }
    ...
    for (i = 0; i < sp->typecnt; ++i) {
      ...
      if (ttisstdcnt == 0) ttisp->tt_ttisstd = FALSE;
      else {
        ttisp->tt_ttisstd = *p++;

        if ((ttisp->tt_ttisstd != TRUE) &&
            (ttisp->tt_ttisstd != FALSE)) return -1;
      }
    }
    ...
    for (i = 0; i < sp->typecnt; ++i) {
      ...
      if (ttisgmtcnt == 0) ttisp->tt_ttisgmt = FALSE;
      else {
        ttisp->tt_ttisgmt = *p++;

        if ((ttisp->tt_ttisgmt != TRUE) &&
            (ttisp->tt_ttisgmt != FALSE)) return -1;
      }
    }
  ...
  if (doextend && (nread > 2) &&
    ...
    if ((result == 0) && (ts.typecnt == 2) &&
      ...
      for (i = 0; i < 2; ++i) ts.ttis[i].tt_abbrind +=
          sp->charcnt;

      for (i = 0; i < ts.charcnt; ++i) sp->chars[sp->charcnt++] =
          ts.chars[i];
      i = 0;
  ...
  case MONTH_NTH_DAY_OF_WEEK:
    ...
    */
    ...
    for (i = 0; i < rulep->r_mon - 1;
         ++i) value += mon_lengths[leapyear][i] * SECSPERDAY;
    ...
    if ((*name == ',') || (*name == ';')) {
      ...
      */
      for (i = 0; i < sp->timecnt; ++i) {
        ...
        if (sp->ttis[j].tt_isdst) theirdstoffset = theiroffset;
...
{
  if (tzload(gmt, sp, TRUE) != 0) return tzparse(gmt, sp, TRUE);
  else return -1;
}
...
static const struct state* ast_tzset(const char *zone)
{
  ...
  if (tzload(zone, sp, TRUE) != 0) {
    if ((zone[0] == ':') || (tzparse(zone, sp, FALSE) != 0)) (void)gmtload(sp);
  }
                               ...
                               const struct state   *sp)
{
  ...
  if ((sp->goback && (t.tv_sec < sp->ats[0])) ||
    ...
    if (t.tv_sec < sp->ats[0]) seconds = sp->ats[0] - t.tv_sec;
    ...
    if (t.tv_sec < sp->ats[0]) newt.tv_sec += seconds;
    else newt.tv_sec -= seconds;

    if ((newt.tv_sec < sp->ats[0]) ||
        (newt.tv_sec > sp->ats[sp->timecnt - 1])) return NULL;  /* "cannot
                                                                   happen" */
    ...
    if (result == tmp) {
      ...
      if (t.tv_sec < sp->ats[0]) newy -= icycles * YEARSPERREPEAT;
                      ...
                      const char *const   zone)
{
  ...
  if (NULL == dst_enabled) return;

  *dst_enabled = 0;

  if ((NULL == dst_start) || (NULL == dst_end) || (NULL == gmt_off)) return;
  ...
  *gmt_off = 0;
  ...
  if ((sp->goback && (t < sp->ats[0])) ||
    ...
    if (t < sp->ats[0]) seconds = sp->ats[0] - t;
    ...
    if ((tcycles - icycles >= 1) || (icycles - tcycles >= 1)) return;
    ...
    if ((t < sp->ats[0]) || (t > sp->ats[sp->timecnt - 1])) return;  /* "cannot
                                                                        happen"
                                                                        */
  ...
  /* if I found transition times that do not bounded the given time and these
     correspond to
          or the bounding zones do not reflect a changes in day light savings,
             then I do not have dst active */
  if ((i >= sp->timecnt) || (0 > transition1) || (0 > transition2) ||
    ...
    *gmt_off     = sp->ttis[sp->types[sp->timecnt - 1]].tt_gmtoff;
    ...
    if (sp->ttis[transition2].tt_isdst) *gmt_off =
        sp->ttis[transition1].tt_gmtoff;
    else *gmt_off = sp->ttis[transition2].tt_gmtoff;

    /* If I adjusted the time earlier, indicate that the dst is invalid */
    if (!bounds_exceeded) {
      *dst_enabled = 1;

      /* Determine which of the bounds is the start of daylight savings and
         which is the end */
                             ...
                             struct ast_tm        *tmp)
{
  ...
  */
  if (offset != 0) tmp->TM_ZONE = "    ";
  else tmp->TM_ZONE = sp->chars;
#endif /* defined TM_ZONE */
  return result;
}
                              ...
                              struct ast_tm        *tmp)
{
  ...
  */
  ...
  for (tmp->tm_mon = 0; idays >= ip[tmp->tm_mon];
       ++(tmp->tm_mon)) idays -= ip[tmp->tm_mon];
...
static int tmcomp(const struct ast_tm *atmp, const struct ast_tm *btmp)
{
  ...
  if (((result = (atmp->tm_year - btmp->tm_year)) == 0) &&
      ...
      ((result = (atmp->tm_sec - btmp->tm_sec)) ==
       0)) result = atmp->tm_usec - btmp->tm_usec;
  return result;
}

static struct timeval time2sub(struct ast_tm *tmp, struct ast_tm * (*const funcp)(
                                 const struct timeval *,
                                 long,
                                 struct ast_tm *,
                                 const struct state *), const long offset, int *okayp, const int do_norm_secs,
                               const struct state *sp)
{
  int dir;
  int i, j;
  ...
  */
  ...
  if ((yourtm.tm_sec >= 0) && (yourtm.tm_sec < SECSPERMIN)) saved_seconds = 0;
  ...
  */
  if (!TYPE_SIGNED(time_t)) {
    ...
    if (sizeof(time_t) > sizeof(float)) hi = (time_t)DBL_MAX;
    else hi = (time_t)FLT_MAX;
    lo = -hi;
  } else {
    lo = 1;

    for (i = 0; i < (int)TYPE_BIT(time_t) - 1; ++i) lo *= 2;
    hi = -(lo + 1);
  }
  ...
  for (;;) {
    ...
    if (t.tv_sec < lo) t.tv_sec = lo;
    else if (t.tv_sec > hi) t.tv_sec = hi;
    ...
    if (dir != 0) {
      if (t.tv_sec == lo) {
        ...
        if (t.tv_sec <= lo) return WRONG;

        ++lo;
      } else if (t.tv_sec == hi) {
        --t.tv_sec;

        if (t.tv_sec >= hi) return WRONG;
    ...
    */
    for (i = sp->typecnt - 1; i >= 0; --i) {
      ...
      for (j = sp->typecnt - 1; j >= 0; --j) {
        ...
        if ((*funcp)(&newt, offset, &mytm, sp) == NULL) continue;
...
label:
  ...
  if ((newt.tv_sec < t.tv_sec) != (saved_seconds < 0)) return WRONG;

  t.tv_sec = newt.tv_sec;

  if ((*funcp)(&t, offset, tmp, sp)) *okayp = TRUE;
  return t;
}

static struct timeval time2(struct ast_tm *tmp, struct ast_tm * (*const funcp)(
                              const struct timeval *,
                              long,
                              struct ast_tm *,
                              const struct state *sp), const long offset, int *okayp,
                            const struct state *sp)
{
  struct timeval t;
...
static struct timeval time1(struct ast_tm *tmp, struct ast_tm * (*const funcp)(
                              const struct timeval *,
                              long,
                              struct ast_tm *,
                              const struct state *), const long offset,
                            const struct state *sp)
{
  struct timeval t;
  int samei, otheri;
  ...
  */
  ...
  for (i = 0; i < sp->typecnt; ++i) seen[i] = FALSE;
  nseen = 0;
...
struct timeval ast_mktime(struct ast_tm *tmp, const char *zone)
{
  ...
  if (!(sp = ast_tzset(zone))) return WRONG;
