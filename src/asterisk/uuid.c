 ...
 *
 * \extref Depends on libuuid, a component of the e2fsprogs package -
...
static void generate_uuid(struct ast_uuid *uuid)
{
   ...
   * attempts to use either /dev/urandom or /dev/random to generate random
   * values.
   ...
   *
   *     a) libuuid assumes a hardcoded location for the .pid file of the
   * daemon.
   *     However, the daemon could already be running on the system in a
   * different
   ...
   *     time-based UUID will result in launching the daemon. Because of the
   * hard-
   ...
   *
   *     c) Once the daemon is running, concurrent requests for UUIDs are
   * thread-safe.
   *     However, the actual launching of the daemon is not thread-safe since
   * libuuid
   *     uses no synchronization primitives to ensure that only one thread (or
   * process)
   *     launches the daemon.
   *
   ...
   *     then the daemon will exit. If a new request should occur after the
   * daemon
   *     exits, then the daemon will be relaunched. Given point c), we cannot
   *     necessarily guarantee the thread-safety of time-based UUID generation
   * since
   *     we cannot necessarily guarantee the daemon is running as we expect.
   *     We could set up a watchdog thread to generate UUIDs at regular
   * intervals to
   ...
   *
   * Given these drawbacks, we stick to only using random UUIDs. The chance of
   * /dev/random
   * or /dev/urandom not existing on systems in this age is next to none.
   */

  /* XXX Currently, we only protect this call if the user has no /dev/urandom on
     their system.
   * If it turns out that there are issues with UUID generation despite the
   * presence of
