...
static short convshort(char *s)
{
  ...
  if ((a == 0377) && (b == 0377)) return -1;

  if ((a == 0376) && (b == 0377)) return -2;
...
int ast_term_init(void)
{
  ...
  if (!ast_opt_console) {
    /* If any remote console is not compatible, we'll strip the color codes at
       that point */
    vt100compat = 1;
    goto end;
  }
...
char* term_color_code(char *outbuf, int fgcolor, int bgcolor, int maxout)
{
  ...
  if (ast_opt_force_black_background) {
    snprintf(outbuf, maxout, "%c[%d;%d;%dm", ESC, attr, fgcolor,
             COLOR_BLACK + 10);
  } else if (bgcolor) {
...
char* term_strip(char *outbuf, const char *inbuf, int maxout)
{
  ...
  while (outbuf_ptr < outbuf + maxout) {
    ...
    case ESC:
      ...
      while (*inbuf_ptr && (*inbuf_ptr != 'm')) inbuf_ptr++;
      break;
