...
struct io_context* io_context_create(void)
{
  ...
  if (!(tmp = ast_malloc(sizeof(*tmp)))) return NULL;
...
void io_context_destroy(struct io_context *ioc)
{
  ...
  if (ioc->fds) ast_free(ioc->fds);

  if (ioc->ior) ast_free(ioc->ior);
                   ...
                   void              *data)
{
  ...
  if (*id > ioc->fdcnt) return NULL;

  if (fd > -1) ioc->fds[*id].fd = fd;

  if (callback) ioc->ior[*id].callback = callback;

  if (events) ioc->fds[*id].events = events;

  if (data) ioc->ior[*id].data = data;
...
int ast_io_remove(struct io_context *ioc, int *_id)
{
  ...
  for (x = 0; x < ioc->fdcnt; x++) {
    if (ioc->ior[x].id == _id) {
      ...
      if (ioc->current_ioc == -1) io_shrink(ioc);
      return 0;
    }
  }
...
int ast_io_wait(struct io_context *ioc, int howlong)
{
  ...
  for (x = 0; x < origcnt; x++) {
    ...
    if (ioc->fds[x].revents && ioc->ior[x].id) {
      ...
      if (ioc->ior[x].callback) {
        if (!ioc->ior[x].callback(ioc->ior[x].id, ioc->fds[x].fd,
                                  ioc->fds[x].revents, ioc->ior[x].data)) {
  ...
  if (ioc->needshrink) io_shrink(ioc);
...
void ast_io_dump(struct io_context *ioc)
{
  ...
  ast_debug(1,
               "Asterisk IO Dump: %u entries, %u max entries\n",
            ioc->fdcnt,
            ioc->maxfdcnt);
  ast_debug(1, "================================================\n");
...
int ast_get_termcols(int fd)
{
  ...
  if (ioctl(fd, TIOCGWINSZ, &win) != -1) {
    if (!cols && (win.ws_col > 0)) cols = (int)win.ws_col;
  } else {
