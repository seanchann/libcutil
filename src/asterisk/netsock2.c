...
{
  char *s        = str;
  char *orig_str = str;  /* Original string in case the port presence is
                            incorrect. */
  char *host_end = NULL; /* Delay terminating the host in case the port presence
                            is incorrect. */
...
int ast_sockaddr_parse(struct ast_sockaddr *addr, const char *str, int flags)
{
  ...
  if ((e = getaddrinfo(host, port, &hints, &res))) {
    if (e != EAI_NONAME) { /* if this was just a host name rather than a ip
                              address, don't print error */
...
int ast_sockaddr_is_any(const struct ast_sockaddr *addr)
{
  ...
  return (ast_sockaddr_is_ipv4(addr) &&
          (tmp_addr.sin.sin_addr.s_addr == INADDR_ANY)) ||
         (ast_sockaddr_is_ipv6(addr) &&
          IN6_IS_ADDR_UNSPECIFIED(&tmp_addr.sin6.sin6_addr));
}
...
int ast_set_qos(int sockfd, int tos, int cos, const char *desc)
{
  ...
  set_tclass =
    !ast_getsockname(sockfd, &addr) && ast_sockaddr_is_ipv6(&addr) ? 1 : 0;

  /* If the the sock address is IPv4 or (IPv6 set to any address [::]) set TOS
     bits */
  ...
  if (set_tos) {
    if ((res = setsockopt(sockfd, IPPROTO_IP, IP_TOS, &tos, sizeof(tos)))) {
      ast_log(LOG_WARNING,
              "Unable to set %s DSCP TOS value to %d (may be you have no "
              "root privileges): %s\n",
              desc,
              tos,
              strerror(errno));
  ...
  if (set_tclass) {
    if (!ast_getsockname(sockfd, &addr) && ast_sockaddr_is_ipv6(&addr)) {
      if ((res =
             setsockopt(sockfd, IPPROTO_IPV6, IPV6_TCLASS, &tos, sizeof(tos)))) {
        ast_log(LOG_WARNING,
                "Unable to set %s DSCP TCLASS field to %d (may be you have no "
                "root privileges): %s\n",
                desc,
                tos,
                strerror(errno));
...
#endif /* if defined(IPV6_TCLASS) && defined(IPPROTO_IPV6) */
