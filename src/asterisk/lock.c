...
static inline struct ast_lock_track* ast_get_reentrancy(
...
# if defined(DO_CRASH) || defined(THREAD_CRASH)
    abort();
# else /* if defined(DO_CRASH) || defined(THREAD_CRASH) */
    pthread_mutex_unlock(&reentrancy_lock.mutex);
    return NULL;
...
#endif /* ifdef HAVE_PTHREAD_RWLOCK_PREFER_WRITER_NP */
...
# endif /* ifdef HAVE_BKTR */
            __ast_mutex_logger("%s line %d (%s): '%s' was locked  here.\n",
                               lt->file[lt->reentrancy - 1],
                               lt->lineno[lt->reentrancy - 1],
...
# endif /* ifdef HAVE_BKTR */
...
#else /* ifdef HAVE_PTHREAD_RWLOCK_TIMEDWRLOCK */
  ...
  do {
    ...
    for (;;) {
      ...
      if ((_now.tv_sec > abs_timeout->tv_sec) ||
          ((_now.tv_sec == abs_timeout->tv_sec) &&
           (_now.tv_usec * 1000 > abs_timeout->tv_nsec))) {
        break;
      }
      usleep(1);
    }
  } while (0);
#endif /* ifdef HAVE_PTHREAD_RWLOCK_TIMEDWRLOCK */
