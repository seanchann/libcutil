...
struct hostent* ast_gethostbyname(const char *host, struct ast_hostent *hp)
{
  ...
  if (!s || !*s) {
    ...
    if (inet_pton(AF_INET, host, hp->hp.h_addr) > 0) return &hp->hp;
...
void ast_md5_hash(char *output, const char *input)
{
  ...
  for (x = 0; x < 16; x++) ptr += sprintf(ptr, "%02hhx", digest[x]);
}
...
void ast_sha1_hash(char *output, const char *input)
{
  ...
  for (x = 0; x < 20; x++) ptr += sprintf(ptr, "%02hhx", Message_Digest[x]);
}
...
# if !defined(LOW_MEMORY)
  ...
  for (i = lock_info->num_locks - 1; i >= 0; i--) {
    if (lock_info->locks[i].lock_addr == lock_addr) break;
  }
...
/*! This function can help you find highly temporal locks; locks that happen for
   a
    short time, but at unexpected times, usually at times that create a
       deadlock,
        Why is this thing locked right then? Who is locking it? Who am I
           fighting
    with for this lock?

        To answer such questions, just call this routine before you would
           normally try
        to aquire a lock. It doesn't do anything if the lock is not acquired. If
           the
        lock is taken, it will publish a line or two to the console via
           ast_log().

        Sometimes, the lock message is pretty uninformative. For instance, you
           might
        find that the lock is being aquired deep within the astobj2 code; this
           tells
                             ...
                             const char     *start_fn)
{
  ...
  /* On Linux and FreeBSD , pthread_attr_init() defaults to
                    ...
                    char              delim)
{
  ...
  for (x = 0; ofs < len && x < size && w[x]; x++) {
    ...
    for (src = w[x]; *src && ofs < len; src++) s[ofs++] = *src;
  }
                    ...
                    int            *consumed)
{
  ...
  *dst = _default;
  ...
  if (ast_strlen_zero(src)) return -1;
...
int ast_mkdir(const char *path, int mode)
{
  ...
  *fullpath = '\0';
  ...
  for (x = 0; x < piececount; x++) {
    ...
    if (res && (errno != EEXIST)) return errno;
  }
  return 0;
}
...
int ast_parse_digest(const char             *digest,
  ...
  if (!request) {
    ...
    if (ast_strlen_zero(d->username) || ast_strlen_zero(d->uri) ||
        ast_strlen_zero(d->response)) {
      return -1;
    }

    if (pedantic && d->qop &&
        (ast_strlen_zero(d->cnonce) || ast_strlen_zero(d->nc))) {
      return -1;
    }
  }
...
int ast_get_tid(void)
{
  ...
  ret = lwpid;
#endif /* if defined(__linux) && defined(SYS_gettid) */
  return ret;
}
