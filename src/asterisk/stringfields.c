...
ast_string_field __ast_string_field_empty =
  __ast_string_field_empty_buffer.string;
 ...
 * \param pool_head
 * \param cleanup_type
 *           0: Reset all string fields and free all pools except the last or
 *embedded pool.
 *              Keep the internal management structures so the structure can be
 *reused.
 *          -1: Reset all string fields and free all pools except the embedded
 *pool.
                                   ...
                                   const char                       *func)
{
  ...
  case AST_STRINGFIELD_DESTROY:
    ...
    if (mgr->embedded_pool) { /* ALWAYS preserve the embedded pool if there is
                                 one */
                            ...
                            const char                    *func)
{
  ...
  size_t initial_vector_size = ((size_t)(((char *)mgr) - ((char *)p))) /
                               sizeof(*p);

  if (needed <= 0) {
    return __ast_string_field_free_memory(mgr,
                                          pool_head,
                                          needed,
                                          file,
                                          lineno,
...
#endif /* if defined(__AST_DEBUG_MALLOC) */
...
ast_string_field __ast_string_field_alloc_space(
  ...
  size_t                         needed)
{
  ...
  if (__builtin_expect((to_alloc > space), 0)) {
    ...
    if (add_string_pool(mgr, pool_head, new_size, mgr->owner_file,
                        mgr->owner_line, mgr->owner_func)) return NULL;

#else  /* if defined(__AST_DEBUG_MALLOC) */

    if (add_string_pool(mgr, pool_head, new_size, __FILE__, __LINE__,
                                     ...
                                     va_list                        ap)
{
  ...
  if (*ptr != __ast_string_field_empty) {
    ...
    /* pool->used is always a multiple of
       ast_alignof(ast_string_field_allocation)
     * so we don't need to re-align anything here.
     */
    target = (*pool_head)->base + (*pool_head)->used + ast_alignof(
  ...
  if (needed > available) {
    /* the allocation could not be satisfied using the field's current
       allocation
       (if it has one), or the space available in the pool (if it does not).
          allocate
       space for it, adding a new string pool if necessary.
     */
    if (!(target =
            (char *)__ast_string_field_alloc_space(mgr, pool_head, needed))) {
      return;
    }
    ...
    *ptr = target;
    ...
    (*pool_head)->used                 += ast_make_room_for(needed,
                                                            ast_string_field_allocation);
    (*pool_head)->active               += needed;
     ...
     */
    AST_STRING_FIELD_ALLOCATION(*ptr) += grow;
    (*pool_head)->used                += ast_align_for(grow,
                                                       ast_string_field_allocation);
    (*pool_head)->active              += grow;
  }
}
                                     ...
                                     const char  *func)
{
  ...
  if (!(allocation =
          calloc_wrapper(num_structs, size_to_alloc, file, lineno, func))) {
    return NULL;
  }
                             ...
                             struct ast_string_field_mgr  *orig_mgr)
{
  ...
  for (i = 0; i < AST_VECTOR_SIZE(dest); i++) {
    if (ast_string_field_ptr_set_by_fields(copy_pool, *copy_mgr,
